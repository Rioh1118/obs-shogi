...
| `index_cache_guard_names`（Rust） | 索引キャッシュの**書き側**の門番と、それを名乗るテストの食い違い。見る向きは3つ——規約の外の綴り（`_rejected` など）で名乗るテストがある／`encode_all` の `refusing to write:` の文言の**最長の固定部分**を `contains` で見る書き側のテストが無い／その固定部分が短すぎて検査が恒真になる | 無し。綴りは `_not_written` / `_refused` / `_neither_written_nor_read` のどれかにする。文言には書式指定の間に句を1つ入れる。**同じ句を読み書き両側で使う門番は、片側の assert を消しても落ちない**ので、そこは人が見る                                                   |
