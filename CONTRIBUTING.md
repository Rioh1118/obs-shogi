...
| 暗い面の上に置く文字   | `$color-danger-text` / `$color-fatal-text`          |
| 失敗を伝える箱の面     | `$surface-info` / `-warning` / `-danger` / `-fatal` |

失敗の箱を新しく作るときは `$surface-*` を使ってください。
`color-mix(...)` を手で書くと、同じ式が複数のファイルに散ります。

**`$color-danger` と `$color-fatal` は暗い面の上に直接置けません。**
それぞれ 3.75:1 と 2.32:1 で、`fatal` は輪郭の基準（3:1）も割ります。
帯・記号・文言に使うときは `-text` の側を選んでください。
持ち上げた2つは色が近くなりますが（元の色相が同じなので避けられません）、
