...
| `index_cache_guard_names`（Rust） | 索引キャッシュの門番と、それを名乗るテストの食い違い。見る向きは2つ——規約の外の綴り（`_rejected` など）で名乗るテストがある／`encode_all` の `refusing to write:` の文言を `contains` で見る書き側のテストが無い                                           | 無し。綴りは `_not_written` / `_refused` / `_neither_written_nor_read` のどれかにする。**両側を見るテストの中でどちらの assert かは見分けられない**ので、そこは人が見る                                                                                                  |
