...
| `index_cache_guard_names`（Rust） | 索引キャッシュの**書き側**の門番と、それを名乗るテストの食い違い。見る向きは5つ——既知の綴り（`_rejected` など）で名乗るテストがある／`encode_all` の `return Err` の数と `refusing to write:` の数が合わない／文言の**最長の固定部分**を `contains` で見る書き側のテストが無い／その固定部分が短すぎて検査が恒真になる／ヘッダの検査（版・magic・root hash）が構造の門番の綴りを名乗っている | 無し。綴りは `_not_written` / `_refused` / `_neither_written_nor_read` のどれかにし、文言は `refusing to write:` で始めて書式指定の間に句を1つ入れる。**綴りの検査は既知の3語だけ**なので新しい同義語は素通りする。**同じ句を読み書き両側で使う門番は、片側の assert を消しても落ちない** |
