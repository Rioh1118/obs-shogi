...
人の注意では止まらなかったものを検査として置いてあります（（Rust）の行は `npm run verify:rust` 側、
それ以外は `npm run test`）。落ちたときの逃げ道はそれぞれ違います。
...
| `ratchetIndex`               | 検査を足してこの表に載せ忘れること                                           | 走査器の単体テストなら `SCANNER_TESTS` に名前。ラチェットなら行を足す                                                                                                                                                                                                    |
...
| `stateTransitionIndex`       | 状態遷移表を足して索引に載せ忘れること。参照リンクの定義が使用と食い違うこと | 無し。`docs/state-transitions/README.md` の**在庫表と階層図の両方**に足す。参照リンクは使用と定義を同時に直す                                                                                                                                                            |
...
| `state_table_terms`（Rust）  | 状態遷移表が実在しない定数を仕様として書くこと                               | 表の綴りを直す。Rust に無い表は検査の `NOT_RUST` に、実装の定数でない語は `NOT_IDENTIFIERS` に、理由と一緒に                                                                                                                                                             |
...
| `docsSourcePaths`            | doc が実在しないソースのパスを指すこと                                       | パスを直す。**他リポジトリのパスは外部リンクで書く**（規約は `docs/state-transitions/README.md`）。省略形で追えるのは `src/` 直下のレイヤ名で始まる綴りだけ                                                                                                              |
