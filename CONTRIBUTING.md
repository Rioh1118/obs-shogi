...
| `docsSourcePaths`                  | 状態遷移表と `docs/spec/screens/` が実在しないパスを指すこと／**`docs/` 全体が行番号で指すこと**                                                                                                                                                                                                                                                                                                           | パスに逃げ道は無し（直す。接頭辞を省いた書き方も拾う）。行番号は `EXEMPT` に相対パスと理由——**他リポジトリを引くときだけ**。それ以外は落として識別子で指す                                                                                                                                                                                                                                     |
| `docsIdentifiers`                  | 状態遷移表と `docs/spec/screens/` が実在しない識別子を指すこと                                                                                                                                                                                                                                                                                                                                                                     | `EXEMPT` に1行。ただし**なぜソースに無くてよいか**を書けるときだけ。書けないなら doc が腐っている                                                                                                                                                                                                                                                                                              |
