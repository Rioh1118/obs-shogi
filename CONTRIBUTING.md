...
| `analysisSeatSlot`                 | 解析の席を返す枠を `holdSlot` の外から書き換えること／`useEngineSeat` の早期 return より上に `useRef` 以外の宣言を置くこと                                                                                                                                                                                                                                                                                 | 無し。枠は `holdSlot` を通す。凍る前に置けるのは `useRef` だけ（返す口を初回の描画で凍らせるため）                                                                                                                                                                                                                                                                                             |
