...
| `state_transition_cells`（Rust）   | 状態遷移表と、そのセルを名乗るテストが食い違うこと。見る向きは4つ——テスト列が `✗` なのに名乗るテストがある／「埋まっていないセル」節に載っているのにテスト列が `✓`／テストが実在しない行・注を指す／テスト列が `✓` なのに名乗るテストが無い                                                                                                                                                                | 無し。**テストの doc に書いた `（表の E1 / E4）` が鍵**なので、表かテストのどちらかを現物に合わせる。**`△` の行は突き合わせない**（どの列が固定済みかを表が持てないため）。注は番号の実在だけを見て、本文は読まない                                                                                                                                                                            |
