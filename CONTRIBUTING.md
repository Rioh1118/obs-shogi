...
`src/__tests__/`。**1ファイルの内部の形しか見ないもの**はそのスライスに置き、
`*.ratchet.test.ts` と名乗ってください（`seatSlotShape` / `exportsTested`）。
置き場ではなく綴りで見分けるのは、スライスへ移した瞬間に上の表へ載せる義務から
外れるのを防ぐためです（`ratchetIndex` がその綴りで両方に掛けます）。
