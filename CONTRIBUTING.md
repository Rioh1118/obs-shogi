...
| `comment_identifiers`（Rust）    | `src-tauri/src` の**行頭コメント**が実在しない識別子を指すこと。**および `#[test]` / `#[tokio::test]` の doc に要約が2つ並ぶこと**（関数を挿し込んで前の doc を引き剥がした形）                                                             | 識別子は `EXEMPT` に1行。ただし**なぜソースに無くてよいか**を書けるときだけ。使われない除外は別の検査が落とす。要約の重複に逃げ道は無し（拾えるのは「〜こと。」で終わる行が2本以上あるときだけ）                                                                         |
