...
- 日付: 2026-09-01
- 状態: **提案。ただし決定3（Rust の直置きを無くす）は ADR-0009 が引き取った。**
...
**置き場は ADR-0009 が決めた。** 決定3 のうち、この案と違う結論になったものが2つある。

| この案                                | ADR-0009                      | 理由                                                                                                                                           |
| ------------------------------------- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `config/` `preset/` `study/` を別々に | `settings/` にまとめる        | 3つとも `app_config_dir` に JSON を1つ置いて読み書きするだけで、形が同じ。分けていると片方だけ直した違いが「どの設定がどう壊れるか」の差になる |
| `file_system/` → `tree/`              | `file_system/` → `workspace/` | この枝が持つのはツリーだけではない。root の関門・作成・移動・削除も同じ場所にある                                                              |
