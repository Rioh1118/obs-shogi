...
| 境界                                    | 畳まれる範囲                                       |
| --------------------------------------- | -------------------------------------------------- |
| `App` の `.app-root` の内側             | **最後の砦。** 窓の中身の全部                      |
| `RuntimeShell` の `.app-content` の内側 | `/app` の中身。ヘッダとサイドバーもここ            |
| `AppModalLayer` を包む（`AppLayout`）   | モーダル1枚のレンダ例外で本体まで unmount させない |
| 盤ペインを包む（`AppLayout`）           | 盤と操作列。棋譜一覧と解析は残る                   |
| `KifuStreamList` を包む（`AppLayout`）  | 棋譜一覧の例外で盤まで落とさない                   |
| 解析ペインを包む（`AppLayout`）         | 解析ペイン。盤は残る                               |

**いちばん内側の境界が受ける**ので、畳まれるのは落ちた部分だけ。

**枠が残るかは、どの境界が受けたかで決まる。** ウィンドウ枠は自前（`decorations: false`）で
`TitleBar` は `RuntimeShell` の中に居るので、`.app-content` 以下で受けたぶんには本物の枠が残る。
root まで上がるのは provider・guard・ルータ自身が落ちたときだけで、そのときは
`RootErrorFallback` が**自前のドラッグ領域と閉じるボタン**を描く。
