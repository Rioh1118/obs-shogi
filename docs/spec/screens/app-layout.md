...
**畳んだままになる／戻らない場合がある。** `再表示` は境界の `error` を消すだけなので、
原因が境界の外（provider の state）にあれば同じ行で落ち直し、画面は変わらない。
逆に原因が消えても境界はそれを知らないので、`resetKeys` を渡している境界以外は畳んだままになる。
**何を鍵にしているかは各境界の呼び出し側が持つ**（モーダル層は `useModalLayerResetKeys`）。
ここに写さない。
