...
| アプリのシェル           | [screens/app-layout.md](screens/app-layout.md)       | `pages/AppLayout.tsx` `widgets/dock/`（境界は「失敗の見せ方」） |
...
| 対局                       | [features/game-play.md](features/game-play.md)           | #374 ほか     | 始める面と進行を出す面まで。人が指せない |
...
**画面がまだ無いので、上の順は使えない。** 代わりに次を必ず持たせる。

- 冒頭に**追跡する issue 番号**と、**main にあるか**の1行
- **いまどうなっているか** — 部分的にできることがあるなら、その境目
- **受入条件** — issue から引く。書き換えない
- **触ることになる画面** — `screens/` へのリンク
- **決まっていないこと** — `docs/OPEN-QUESTIONS.md` の問いへ繋ぐ
- **判断の軸** — `docs/PREMISES.md` のどの前提に乗っているか

在庫（未マージのブランチ）があるなら**どのブランチに何があるか**も書く。

### 守ること

- **`screens/` に書くパスと識別子は実在するものだけ。** これから作る置き場は `features/` に書き、
  `screens/` からはそこへリンクする（走査の範囲は `src/__tests__/docsSourcePaths.ts` が決める）。
  他実装の綴りをどうしても引くときは `src/__tests__/docsIdentifiers.ts` の免除へ、
  **なぜこちらのソースに無くてよいかを書けるときだけ**足す
