...
| **E2**  | 設定の読み書きが**失敗する** | 起動時ロード・保存・ワークスペース選択 |
...
| **A0** | → A1/A2     | **`/` に戻す**※1   | —                                        | —                                            | —               | —                                | —                              | —                     | —                                  | —                           |
| **A1** | —           | —                  | → A2                                     | —                                            | —               | 設定は保存される                 | —                              | —                     | —                                  | —                           |
| **A2** | —           | **`/` に戻す**※1   | ツリー再読込 → [file-tree](file-tree.md) | → A3 → [game](game.md)                       | —               | → [engine](engine.md)            | → A2（棋譜が無くても起動する） | → [engine](engine.md) | —                                  | → [file-tree](file-tree.md) |
| **A3** | —           | **`/` に戻す**※1   | → A2（棋譜を閉じる）※2                   | 別の棋譜へ                                   | → A2            | → [engine](engine.md)            | → **A4**                       | —                     | 開始できない※3                     | → [file-tree](file-tree.md) |
| **A4** | —           | **`/` に戻す**※1※5 | → A2                                     | 別の棋譜へ → [sync](engine-position-sync.md) | → A2            | 再起動 → [engine](engine.md)     | —                              | → **A3**              | → **A5** → [analysis](analysis.md) | → [file-tree](file-tree.md) |
| **A5** | —           | **`/` に戻す**※1※5 | → A2※4                                   | 局面が変わる → [analysis](analysis.md)       | → A2※4          | 再起動 → [analysis](analysis.md) | —                              | → A3※4                | → A4                               | → [file-tree](file-tree.md) |

### 注

※1 `RequireRootDir` が `error` を見て `<Navigate to="/" replace />`。

**`error` を立てる口**は起動時ロードと `updateConfig` / `chooseRootDir` / `setLastPresetId`
（`entities/app-config/model/provider.tsx`）。`setRootDir`（ツリーの根のリネーム）と
`chooseAiRoot` は**意図して立てない**。立てると、呼び手が出そうとした失敗が画面に出る前に
ランタイムごと畳まれるので、あの2つは `settled` を降ろし、失敗は呼び手がその場に出す。
**ここを `error` に変えてはいけない。**
...
※5 設定の保存やワークスペースの選び直しが落ちると、**起動後でも `error` が立つ**
