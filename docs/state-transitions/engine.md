...
| `starting`  | 残り全部                                                      | 戻る（S1 の起動待ち、S2 で runtime が変わった窓、畳み損ねて S0 に落ちた窓※、**S3 で runtime が動いた窓**、**S3 で前回試した値が無い窓**（`initialize` を通らずに `error` へ入った回。いまその口は無い）） |
...
**畳めなかったことは `failed` として出ない。** 畳む側は失敗を返さず
（`EngineAnalyzer::shutdown` は常に `Ok`）、フロントも `idle` へ落として
そこから必ず起こし直す（下の effect の `idle` の枝）。**残ったプロセスの話は
※3 / F-8 の管轄**で、この理由の分類とは別。

したがって `starting` は必ず `ready` か `failed` へ動く。**待てば理由が動く**
という意味で戻る側でよい（読み手はそこで諦められる）。F-39 が言っているのも
同じこと——**`starting` に居座る窓ではなく、`failed` へ落ちるまでの待ちが長い**という話。
