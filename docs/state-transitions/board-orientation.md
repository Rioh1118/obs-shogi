...
※3 **B3 で受けたイベントは、B3 の結末を変えない。** B3 はレンダとエフェクトの間だけの
状態で、保留されているエフェクトは合図の**現在値**を読んで走る。その間に別のイベントが
来ても、React はエフェクトを走らせてから次のレンダを描く。結末は必ず B0 か B1 で、
どのイベントを挟んだかに依らない。だから空欄ではなく `—`。
