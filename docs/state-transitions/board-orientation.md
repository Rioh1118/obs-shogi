...
| 記号   | 状態         | 判定                                                                                  |
| ------ | ------------ | ------------------------------------------------------------------------------------- |
| **B0** | 盤に何も無い | `shownKifuPathRef.current === loadedAbsPath === null`※6                               |
...
| **E2** | 棋譜を閉じる                               | 開いている棋譜（か親）の削除・ツリーからの消失・ルート外への移動 → `kifu_closed` → `resetGame`※5 |
...
| セル       | 何が抜けているか                                                                                     |
| ---------- | ---------------------------------------------------------------------------------------------------- |
| `(B2, E7)` | **開いている棋譜を改名・移動すると、盤ごと開いた時点まで戻り、向きも落ちる。** テストも無い※2 → #262 |
