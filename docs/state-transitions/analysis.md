...
| --------- | ------------------------------- | --------------------------------------------------- | ---------------- | ----------- | --------------------------------------------------- | --------------------------------------------------------------- | ----------------- | ---------------------- | -------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------- | ---------------------------------------------- | ----------------------------------------------------- | ----------------------------------------------------- |
| **S0/P0** | 同期→開始※2                     | `stop_analysis` を出さず state だけ落とす           | —                | —           | → S6（※2 の待ちが上限に達した回。席は空 → ※4）      | —                                                               | 捨てる※3          | —                      | → S6                 | → S0 のまま※4                                                            | —                                                             | 結果が二度と届かないが画面には何も出ない → F-4 | —                                                     | —                                                     |
| **S0/P1** | 握っていれば返してから始める※11 | 握っていれば席を返す。応答待ちでは state だけ落とす | —                | —           | —                                                   | —                                                               | 捨てる※3          | —                      | → S6/P1 のまま※1     | エンジン側の失敗なら → S0/**P0**※4。席が残っていた回は → S0/P1 のまま※11 | ▶ の先頭の返却が落ちた回 → S6・**席は握ったまま**（※2 / ※15） | —                                              | 開始の応答待ちなら 席を返す※13。※7 の後は 席を返す※12 | 開始の応答待ちなら 席を返す※13。※7 の後は 席を返す※14 |
...
- **先に席を返す要求を出してから撃つ**——自動再開の同期の打ち切り（→ ※9）、
  ...
  返せたかどうかは別（→ ※9 / ※12）
- **席を握っていない窓で撃つ**ので要求も出さない——▶ の前置きの門（エンジンが
  ready でない／読む局面が無い）と、席を返し終えた後の失敗（送信・打ち切り・開始の拒否。→ ※15）。
...
| `RESTART_FAILED_MESSAGE`        | 自動再開が落ちた（→ `(S4/S5, E10/E11)`） | まず ▶。それでも駄目なら起こし直す     |
