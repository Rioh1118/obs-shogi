...
|           | E1 開始             | E2 停止                                   | E3 局面変化      | E4 同期完了 | E5 同期TO         | E6 not ready      | E7 結果           | E8 完了                | E9 エラー通知        | E10 開始失敗           | E11 停止失敗              | E13 畳まれる     |
| --------- | ------------------- | ----------------------------------------- | ---------------- | ----------- | ----------------- | ----------------- | ----------------- | ---------------------- | -------------------- | ---------------------- | ------------------------- | ---------------- |
...
（`provider.tsx`）。Rust 側は `stop_all_sessions` に落ちて台帳を空にする。
指さないのは、畳まれた瞬間に席に居るのが `sessionIdRef` の1本とは限らないため
（※13）。指した上で別の席が居ると `stop_session` は照合して断る。

**撃つ条件は `isAnalyzing || sessionId !== null`。** `isAnalyzing` だけを見ると
S6（※1 でフロントだけ「停止」になった状態）で席を置き去りにする。
両方が空なら撃たない——StrictMode の setup → cleanup → setup が、
席を持っていない状態で停止を投げるのを避ける

※13 開始の応答（`startInfiniteAnalysisCore`）を待っている間に畳まれると、
席は**後始末が走り終えた後に**作られる。※12 はその席を知らないので、
応答が返った側で `unmountedRef` を見て `stop_analysis` を撃つ（`provider.tsx`）。
手動開始（E1）と自動再開の両方が同じ形。
**E2（停止ボタン）で同じ窓に入る経路は塞いでいない** → ※10
