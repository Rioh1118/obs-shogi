...
| 記号    | イベント                             | 発生源                                                                                                                  |
| ------- | ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| **E1**  | 開始ボタン                           | `AnalysisControls` → `startInfiniteAnalysis()`                                                                          |
...
| **S1/P1** | 何もしない                      | → S0/P0                                             | → S2             | —           | —                                                   | **→ S1 のまま**／P は畳み方で違う※5                                                                                                                                       | `resultFlushMs` の間引きで反映 | → S0（**P は不明**）※6 | → S6/**P1 のまま**※1 | —                                                                 | → S6・**席は握ったまま**（→ ※7 / ※15）                        | —                                                 | 席を返す※12                                           | 席を返して → S0※14                                    |
...
`syncPosition()` → `waitUntil(syncedSfen === currentSfen, positionSyncTimeoutMs)` → 席を取って `go`、の順
（`provider.tsx`）。
...
※4 `startInfiniteAnalysis` の throw は `AnalysisControls` の `console.error` で終わる。
...
| `ENGINE_STARTING_MESSAGE`       | エンジンの起動待ちに ▶ を押した／**同期待ちの最中**・席を取る往復の最中に起こし直され、**まだ戻っていない**（→ ※13）   | 待ってからもう一度 ▶                                           |
...
1. **フロントが S0（停止中）なら Rust も P0。**
   ...
   **残り続けるのは、停止が Rust に届かなかった回**
   ...
   ※13 のうち**畳まれた後**に落ちた回は不変条件5 の管轄（撃ち直すのはフックの中）。
