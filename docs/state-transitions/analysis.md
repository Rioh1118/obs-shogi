...
| **S0/P1** | 握っていれば返してから始める※11 | 握っていれば席を返す。応答待ちでは state だけ落とす | —                | —           | —                 | —                                   | 捨てる※3          | —                      | —                    | エンジン側の失敗なら → S0/**P0**※4。席が残っていた回は → S0/P1 のまま※11 | —                                                   | —                                              | 開始の応答待ちなら 席を返す※13。※7 の後は 席を返す※12 | 席を握っていれば返す※14 |
| **S1/P1** | 何もしない                      | → S0/P0                                             | → S2             | —           | —                 | **→ S1 のまま**／P は畳み方で違う※5 | 80ms 間引きで反映 | → S0（**P は不明**）※6 | → S6/**P1 のまま**※1 | —                                                                        | → S0 だが **P1 が残る**※7                           | —                                              | 席を返す※12                                           | 席を返して → S0※14      |
...
| **S4/P1** | —                               | → S0※10                                             | → S5             | —           | —                 | **→ S1 のまま**／P は畳み方で違う※5 | —                 | —                      | → S6                 | → S6 + `stop_analysis`                                                   | → S6・**席は握ったまま**（P は枝で違う → ※12）      | —                                              | 席を返して → S0※14・※13                               | 席を返して → S0※14      |
| **S5/P1** | —                               | → S0※10                                             | 最新で上書き     | —           | —                 | **→ S1 のまま**／P は畳み方で違う※5 | —                 | —                      | → S6                 | → S6                                                                     | → S6・**席は握ったまま**（P は枝で違う → ※12）      | —                                              | 席を返して → S0※14・※13                               | 席を返して → S0※14      |
| **S6/P0** | 再開できる                      | —                                                   | —                | —           | —                 | —                                   | —                 | —                      | —                    | —                                                                        | —                                                   | —                                              | —                                                     | —                       |
...
プロセスが落ちただけの回は席が残る → [engine.md](engine.md) ※3。
...
1. **フロントが S0（停止中）なら Rust も P0。** 破るのは、**停止が Rust に届かなかった回**
   ...
   どれも席を握ったままにするので、**E1 が返し直せる**（不変条件3）——
   ※7 / `(S3〜S5, E11)` / ※13 は ▶ で、※14（棋譜を閉じた回）は**開き直してから ▶** で。
...
3. **S6（エラー）から抜ける道が常にある。** 席を握っていれば E1 が返してから始めるので抜けられる
   （`provider.tsx`）。**破れているのは、フロントが席の ID を持っていない回だけ**
   ——その回は E1 が※11 に落ちる
