...
| **S0/P1** | 握っていれば返してから始める※11 | 握っていれば席を返す。応答待ちでは state だけ落とす | —                | —           | —                                                   | —                                                                                                                                 | 捨てる※3          | —                      | → S6/P1 のまま※1     | エンジン側の失敗なら → S6/**P0**※4。席が残っていた回は → S6/P1※11 | ▶ の先頭の返却が落ちた回 → S6・**席は握ったまま**（※2 / ※15） | —                                                 | 開始の応答待ちなら 席を返す※13。※7 の後は 席を返す※12 | 開始の応答待ちなら 席を返す※13。※7 の後は 席を返す※14 |
...
判定は `useEngineSeat` の中（`beginTake` が焼き付けたエンジンの世代を `landed` が見る）。
...
**枝を足したらここにも足すこと**（`analysisRefusals` が定数名で突き合わせる）。
