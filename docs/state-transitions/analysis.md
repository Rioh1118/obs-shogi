...
5. **画面が消えたら Rust も P0。** 席の持ち主はこの provider だけで、
   ...
   **畳まれた後の停止が Rust に届かなかった回も、1度だけ撃ち直す。** 落ちた席は
   `keepOrForget` が欄へ戻し、`sweepOnUnmount` が枠の後ろで、または書き戻しの側で
   撃ち直す（画面がもう無いので effect は再走しない。フックの中でしか返せない）。
   **撃ち直しも落ちた回は破れる**——回数を持たないと書き戻しと撃ち直しが回り続けるので、
   1回で止める → F-7。
