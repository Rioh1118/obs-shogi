...
| **S0/P1** | 握っていれば返してから始める※11 | 握っていれば席を返す。応答待ちでは state だけ落とす | —                | —           | —                                              | —                                                               | 捨てる※3          | —                      | → S6/P1 のまま※1     | エンジン側の失敗なら → S0/**P0**※4。席が残っていた回は → S0/P1 のまま※11 | —                                                       | —                                              | 開始の応答待ちなら 席を返す※13。※7 の後は 席を返す※12 | 開始の応答待ちなら 席を返す※13。※7 の後は 席を返す※14 |
...
| **S6/P0** | 同期→開始※2                     | `stop_analysis` を出さず state だけ落とす           | —                | —           | —                                              | —                                                               | 捨てる※3          | —                      | —                    | → S6 のまま                                                              | —                                                       | —                                              | —                                                     | —                                                     |
| **S6/P1** | 握っていれば返してから始める※11 | 握っていれば席を返す                                | —                | —           | —                                              | —                                                               | 捨てる※3          | —                      | —                    | → S6 のまま※11                                                           | —                                                       | —                                              | 席を返す※12                                           | 席を握っていれば返す※14                               |
...
- **先に席を返す要求を出してから撃つ**——自動再開の同期の打ち切り（→ ※9）と、
  自動再開そのものの失敗（→ `(S3〜S5, E11)`）。返せたかどうかは別（→ ※9 / ※12）
- **席を握っていない窓で撃つ**ので要求も出さない——手動開始の失敗（→ ※15）。
  ▶ は先頭で席を返す（※2）ので、そこから先はこの形になる。
  **ただし、その返却そのものが落ちた回は席を握ったまま**（→ 不変条件1）
...
※15 **▶ の失敗は枝ごとに文言が違う**（`provider.tsx`。復帰の手が違うものを同じ1文に
潰すと、読み手——まだ0、→ #277——がどちらを案内するか選べない）。

- **上限まで待っても同期が追いつかない**——押し直しで直りうる
- **局面の送信そのものが落ちた**（`syncPosition` が投げた）——押し直しても同じところで落ちる
- **握っている席を返せない**（→ ※7 / F-7）／**Rust が開始を断った**（→ ※11）
  ——どちらも `runtimeConfig` を動かしてエンジンを起こし直すこと（[engine.md](engine.md) の ※5）
...
1. **フロントが S0（停止中）なら Rust も P0。**
   ...
   **▶ が始める前に出す返却**（※2。落ちると `throw` で終わり、席は握ったまま S0）、
