...
**S0/P1**（フロントは停止・Rust に席が在る）に入る口は3つ。開始の応答待ち
（席は Rust に在るが、フロントはまだ ID を受け取っていない）、※7 の後
（停止に失敗したのにフロントだけ S0 になった）、要らなくなった開始を捨てる停止が
届かなかった回（※12 の2つ目）。
...
| **S0/P1** | 握っていれば返してから始める※11 | 握っていれば席を返す。応答待ちでは state だけ落とす | —                | —           | —                 | —                 | 捨てる※3          | —                      | —                    | —                      | —                                                   | —                                              | 開始の応答待ちなら 席を返す※13。※7 の後は 席を返す※12 | 席を握っていれば返す※14 |
| **S1/P1** | 何もしない                      | → S0/P0                                             | → S2             | —           | —                 | **→ S1 のまま**※5 | 80ms 間引きで反映 | → S0（**P は不明**）※6 | → S6/**P1 のまま**※1 | —                      | → S0 だが **P1 が残る**※7                           | —                                              | 席を返す※12                                           | 席を返して → S0※14      |
| **S2/P1** | —                               | → S0 タイマ破棄                                     | 最新で上書き     | → S4 へ     | —                 | ※5                | 反映              | → S0                   | → S6                 | —                      | —                                                   | —                                              | 席を返す※12                                           | 席を返して → S0※14      |
| **S3/P1** | —                               | → S0                                                | 待ち対象を更新※8 | → S4 へ     | **停止して S6**※9 | ※5                | —                 | —                      | → S6                 | —                      | → S6・**席は握ったまま**（※9 / P は枝で違う → ※12） | —                                              | 席を返す※12                                           | 席を返して → S0※14      |
| **S4/P1** | —                               | → S0※10                                             | → S5             | —           | —                 | ※5                | —                 | —                      | → S6                 | → S6 + `stop_analysis` | → S6・**席は握ったまま**（P は枝で違う → ※12）      | —                                              | 席を返す※12・※13                                      | 席を返して → S0※14      |
| **S5/P1** | —                               | → S0※10                                             | 最新で上書き     | —           | —                 | ※5                | —                 | —                      | → S6                 | → S6                   | → S6・**席は握ったまま**（P は枝で違う → ※12）      | —                                              | 席を返す※12・※13                                      | 席を返して → S0※14      |
| **S6/P0** | 再開できる                      | —                                                   | —                | —           | —                 | —                 | —                 | —                      | —                    | —                      | —                                                   | —                                              | —                                                     | —                       |
| **S6/P1** | 握っていれば返してから始める※11 | —                                                   | —                | —           | —                 | —                 | —                 | —                      | —                    | —                      | —                                                   | —                                              | 席を返す※12                                           | 席を握っていれば返す※14 |
...
※14 棋譜を閉じても解析ペインは畳まれない（`AnalysisProvider` は `RuntimeProviders` 側に居る）。
