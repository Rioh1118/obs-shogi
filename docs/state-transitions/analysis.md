...
| **S0/P1** | 握っていれば返してから始める※11 | 握っていれば席を返す。応答待ちでは state だけ落とす | —                | —           | —                 | —                                   | 捨てる※3          | —                      | —                    | → S0/**P0**※4（席は Rust が返す） | —                                                   | —                                              | 開始の応答待ちなら 席を返す※13。※7 の後は 席を返す※12 | 席を握っていれば返す※14 |
...
| **S4/P1** | —                               | → S0※10                                             | → S5             | —           | —                 | **→ S1 のまま**／P は畳み方で違う※5 | —                 | —                      | → S6                 | → S6 + `stop_analysis`            | → S6・**席は握ったまま**（P は枝で違う → ※12）      | —                                              | 席を返す※12・※13                                      | 席を返して → S0※14      |
| **S5/P1** | —                               | → S0※10                                             | 最新で上書き     | —           | —                 | **→ S1 のまま**／P は畳み方で違う※5 | —                 | —                      | → S6                 | → S6                              | → S6・**席は握ったまま**（P は枝で違う → ※12）      | —                                              | 席を返す※12・※13                                      | 席を返して → S0※14      |
| **S6/P0** | 再開できる                      | —                                                   | —                | —           | —                 | —                                   | —                 | —                      | —                    | —                                 | —                                                   | —                                              | —                                                     | —                       |
...
以降の E1 は**握っている席を返してから**始める（→ ※2 / 不変条件3）。返せなければ ※11 に落ちる。
...
- **一括停止が席を空けた後に落ちた回。** Rust は `drain()` してからエンジンを止めるので
  （`bridge.rs` の `stop_all_sessions`）、`Err` が返っても席は空。欄には古い ID が残る。
  そこから次の開始が席を取ると、応答が返るまで欄と席がずれる
- **要らなくなった開始を捨てる停止（※13）が届かず、その ID を握り直した回。**
  次の開始が通れば同じずれになる
