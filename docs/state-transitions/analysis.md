...
**判定は理由で行う。どの理由が戻ってくるかの出典は [engine.md](engine.md) の ※7**
——ここに写さない。`starting` だけが戻る側で、断らない。

**時間で切らない。** 戻るかどうかは理由で分かるので、時刻を持つ必要が無い。上限を置くと、
重い評価関数の初期化が遅れただけの健全な起こし直しを断りで殺す。

**盤の下から候補手が消えるとは限らない。** ペインは `isAnalyzing` が false になると
`state.candidates` を見なくなり、**局面ごとのキャッシュ**を出す（`AnalysisPane`）。
落ちる直前の1本はその局面の鍵で既に焼き付いているので、**盤を動かすまで同じ候補手が出続ける**
（鍵の engine の部分は `selectedPresetId` で、オプションを変えただけでは動かない）。
盤を動かせば鍵が外れて何も出ない。**ここで閉じたのは、`isAnalyzing` が true のまま
`pvBaseSfen` が前の局面を指し、別の局面の読み筋が新しい盤の下に描かれる経路。**
「止まった」ことが画面に出るのは丸とタイマーだけで、断りの読み手はまだ0（→ #277 / F-2）。
...
| `NO_ENGINE_WHILE_ANALYZING_MESSAGE`     | **解析中に**エンジンの選択が外れた（→ ※5。同上）                                  | 設定でエンジンを選ぶ                                           |
...
2. **`isAnalyzing` が true なら、結果がいつか届くか、エラーが出るか、利用者が止められる。**
   `isReady` が落ちて戻らない回は ※5 が断りを立てて倒す。**まだ破れるのは2つ。**
