...
※1 `set_error` を撃つ口は4つ（`provider.tsx`）。`isAnalyzing: false` にするが、**席の欄には触らない**
...
4つの内訳は次のとおり。**席の欄に触らないまま撃つのは1つ目だけ**で、それは
いま踏めない（→ E9 の行。`engine-error` を出す口が Rust に無い）。

1. **エラー通知（E9 の `onError`）** ——席を返す要求を出さない。**踏めない**
2. **自動再開の同期の打ち切り**（→ ※9）——先に返す要求を出す
3. **自動再開そのものの失敗**（→ `(S3〜S5, E11)`）——先に返す要求を出す
4. **手動開始の待ちの打ち切り**（※2）——**席を握っていない窓**で撃つので要求も出さない

2 と 3 が返せたかどうかは別（→ ※9 / ※12）。
