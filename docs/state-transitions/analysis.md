...
| **S1/P1** | 何もしない                      | → S0/P0                                             | → S2             | —           | —                                                   | `failed` なら **→ S6**（断って候補手も落とす）。他は **→ S1 のまま**／P は畳み方で違う※5                                       | 80ms 間引きで反映 | → S0（**P は不明**）※6 | → S6/**P1 のまま**※1 | —                                                                 | → S6・**席は握ったまま**（→ ※7 / ※15）                        | —                                                 | 席を返す※12                                           | 席を返して → S0※14                                    |
| **S2/P1** | —                               | → S0 タイマ破棄                                     | 最新で上書き     | → S4 へ     | —                                                   | `failed` なら **→ S6**。他は**タイマは落ちる**（S2 を抜ける）。`isReady` が戻れば張り直す※5                                    | 反映              | → S0                   | → S6                 | —                                                                 | —                                                             | —                                                 | 席を返す※12                                           | 席を返して → S0※14                                    |
| **S3/P1** | —                               | → S0                                                | 待ち対象を更新※8 | → S4 へ     | **停止して S6**※9                                   | `failed` なら **→ S6**。他は**その状態のまま**（`isAnalyzing` は true）／P は畳み方で違う※5                                    | —                 | —                      | → S6                 | —                                                                 | → S6・**席は握ったまま**（※9 / P は枝で違う → ※12）           | —                                                 | 席を返す※12                                           | 席を返して → S0※14                                    |
| **S4/P1** | —                               | → S0※10                                             | → S5             | —           | —                                                   | `failed` なら **→ S6**（飛んでいる再開は世代の門で降りる）。他は in-flight の再開が最後まで走る → S1 か S6／P は畳み方で違う※5 | —                 | —                      | → S6                 | → S6                                                              | → S6・**席は握ったまま**（P は枝で違う → ※12）                | —                                                 | 席を返す※12・※13                                      | 席を返して → S0※14・※13                               |
| **S5/P1** | —                               | → S0※10                                             | 最新で上書き     | —           | —                                                   | `failed` なら **→ S6**（飛んでいる再開は世代の門で降りる）。他は in-flight の再開が最後まで走る → S1 か S6／P は畳み方で違う※5 | —                 | —                      | → S6                 | → S6                                                              | → S6・**席は握ったまま**（P は枝で違う → ※12）                | —                                                 | 席を返す※12・※13                                      | 席を返して → S0※14・※13                               |
...
**戻ってこないと分かった回は、そこで断つ**（`ENGINE_FAILED_WHILE_ANALYZING_MESSAGE` → ※15）。
`isAnalyzing` を倒し、**候補手も落とす**——落とさないと、死んだエンジンが最後に返した1本が
いま見ている盤の下に残り、停止中のペインがそれを合法手として描き続ける（`AnalysisPane` は
`isAnalyzing` が false なら局面ごとのキャッシュを出すので、落とした後に盤を進めれば何も出ない）。

**判定は理由で行う。`failed` だけを終端と読む。**

| 理由        | 終端か   | なぜ                                                                                                                |
| ----------- | -------- | ------------------------------------------------------------------------------------------------------------------- |
| `failed`    | **終端** | `initialize_error` が `phase: "error"` を書き、**同じ設定なら再トライしない**（[engine.md](engine.md)）             |
| `starting`  | 違う     | ready か error に落ちる。ここで断つと普通の起こし直しを毎回殺す                                                     |
| `no-engine` | 違う     | `restart()` は `shutdown()` を通り、それが書く `phase: "idle"` の理由がこれ——**健全な起こし直しの途中に観測される** |

**時間で切る案は採らなかった**（#502 の案 A）。`failed` は時刻を持たずに終端だと言えるし、
上限を置くと、重い評価関数の初期化が遅れただけの健全な起こし直しを断りで殺す。
**残る穴は `starting` のまま戻らない回**（`initialize` の invoke が解決しない）で、
そこは今も不変条件2 を破る。**実測は無い**ので、測れてから直す。
...
| `ENGINE_FAILED_WHILE_ANALYZING_MESSAGE` | **解析中に**初期化が落ちて戻らない（→ ※5。ボタンは押していない）                  | 起こし直す                                                     |
...
2. **`isAnalyzing` が true なら、結果がいつか届くか、エラーが出るか、利用者が止められる。**
   エンジンが戻ってこない回は ※5 が断りを立てて倒す（`failed`）。
   **まだ破れるのは `starting` のまま戻らない回**——`initialize` の invoke が解決しなければ
   理由は動かず、誰も断たない。**実測は無い**（→ ※5）
...
- **`(S2〜S5, E6)` エンジンが ready でなくなる（※5）。** S1 は `provider.test.tsx` が踏んでいる
  （起こし直しの前後・応答待ちに落ちた回・同期待ちに落ちた回・**`failed` で戻らない回**）。
  **`failed` の枝は S4 も踏めている**——「席を返している最中に初期化が落ちても、立てた断りは
  消えない」が、飛んでいる再開の最中に落として断りが残ることを見ている。
  残るのは `(S2, E6)` `(S3, E6)` `(S5, E6)` の3行と、`S2〜S5` の `starting` / `no-engine` の枝
