...
**S3' の列は無い。** どの形も宛先がこのプロジェクトのツリーである場合を書いていて、
別リポジトリなら形に関わらず S3' で `exit 0`（`expect_project` が固定している）。
...
3. **誤発火は許容する。** 余分な検証が走るだけで、素通しより危険が小さい。
   **ただし積んだ操作を畳む綴りは対象にしない。** `--abort` / `--quit` / `--edit-todo` は
   ...
   patch で、手元のツリーではない。** そして競合を抱えていれば検証は必ず落ちるので、
   そこで deny すると畳む手段そのものを取り上げることになる。
