...
| **P2** | **メモリとディスクが食い違う** | `save` が失敗し、かつ `jkf_restored` が**通らなかった**。または上限時間で畳んだあとに本物の書き込みが着地した |
...
- `KifuCommentNote`：失敗したら `baseText` を進めずノートの中に出し、本文を面ごとに預かる。
...
**書き込みは `persistIfPossible` で直列化してある。** Rust 側は並行に走らせるので、
順に流さないと後に着地したほうが勝ち、コメントの保存が分岐の削除をディスク上で取り消す。

列に並んでいる間に宛先は変わりうるので、**荷物と宛先は撃った時点で固定し、
走る時点で変わっていないことを確かめる**。確かめずに走る時点の値だけを見ると、
`persistence` と `loadedAbsPath` は棋譜を切り替えたときに**揃って**動くので門番を素通りし、
前の棋譜が新しく開いたファイルへ丸ごと入る。

**上限時間（15秒）は呼び出し元へ返すぶんにだけ掛かる。** 列は本物が settle するまで待つ
（畳んで次を走らせると、刺さった1本と新しい1本が並行に走って直列化の意味が消える）。
上限を過ぎたときの理由は「応答しません。保存できたかどうか分かりません」で、
**このときは巻き戻さない**（書けたかどうかが分からないため。残しておけば次の書き込みが
ディスクを合わせる）。この1点だけ、`save` が失敗を返しても P1 のままとは限らない。
...
E14 で巻き戻しが飛ぶのは2つ。**待っている間に別の編集や読み込みが入ったとき**
（`expectedJkf` の突き合わせ）と、**上限時間で畳んだとき**（書けたかどうかが分からないので
戻さない）。並行する書き込みが実在するので絵空事ではない
...
| `(G1/P2, E2)` 保存に失敗したまま棋譜を閉じる           | **テスト無し。** 巻き戻しが通れば消える編集そのものが無い。飛んだときだけ永久に消える（`resetGame` は保存を挟まない）。**コメントの下書きは別で、`KifuCommentNote` が面ごとに預かる**              |
