...
|                             | 意味                                                                       | `te` の範囲                                    | 型                                                                             |
| --------------------------- | -------------------------------------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------ |
| `state.cursor.forkPointers` | **辿った**変化。いま盤に出ている局面をここまで再生するのに使った選択       | `te <= cursor.tesuu` に必ず正規化される        | `KifuCursor`。`cursorFromPlayer` が作る                                        |
...
`persistIfPossible` の2つ（保存先が無い／保存先が切り替わった）は P0 と
宛先の食い違いの話で、E ではなく P の側で扱う。**残りは** E14 / E16 / E17 と
E3〜E10 のどれかに対応する。
...
| -------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------ |
| **E1** `loadGame`          | → G1           | → G1（前の計画は消える）                                                                                                                                                                                               | → G1（同左）                                                                                         | ✓※8    |
| **E2** `resetGame`         | —              | → G0                                                                                                                                                                                                                   | → G0                                                                                                 | ✗      |
| **E3** `nextMove`          | 無視           | いま辿っている線を1手進む                                                                                                                                                                                              | `te = tesuu+1` の計画があればそこへ降りる。壊れた計画は捨てて線をそのまま1手。線の末尾では動かない※1 | △※7    |
...
| **E6** `goToEnd`           | 無視           | いま辿っている線の葉まで                                                                                                                                                                                               | 計画に沿って降りた葉 → G1。壊れた計画は捨てて線をそのまま進み、葉で止まる※1                          | △※7    |
...
**宛先は state にも残る。** `loadFailedAbsPath` に載り、消えるのは **E1（`game_loaded`）と
E2（`reset_state`）だけ**。`loadFailedSeq` はそのたびに1つ進み、**戻らない**——
その棋譜が載るのを待っている側（`usePositionHitNavigation`）が、要求を出した時点の値と
比べて「自分の要求が落ちたか」を見る。回数まで見ないと、前の回の失敗が残っている状態で
同じ棋譜を要求したときに、まだ走っている試行を「もう来ない」と読む。
...
`KifuStreamList` が棋譜の変化で開いている面を閉じ、`faceKey`（ノートの `face.key`）に
棋譜の識別子を混ぜて Lexical を作り直すが、**エディタを作り直す前に autosave が撃つ競合が残る**ので
この突き合わせが要る。→ #204
