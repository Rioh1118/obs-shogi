...
| **F-3**   | ファイル操作の失敗（作成/リネーム/削除/移動）            | `file-tree` → `FileTreeErrorNotice`      | ツリーは残したままモーダルで知らせる。**対応済み（#169）**            | 要る                   | 読み直し／ワークスペース選び直し | 閉じるまで              |
...
| **F-14**  | リネーム名が不正                                         | `InlineNameEditor`                       | 入力欄の直下に出す。入力は残る。**対応済み（#169）**                  | 要る（**入力の訂正**） | 入力を直す                       | 打ち直すまで            |
