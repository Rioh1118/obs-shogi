...
| **F-12a** | 棋譜の**保存**失敗（`persistIfPossible`）                | `game` `set_error` ×1                    | state に載るが**読み手0**                                             | 要る                   | 再保存                           | ダメ                    |
| **F-12b** | 棋譜の**操作**失敗（移動・分岐入替・分岐削除・盤面選択） | `game` `set_error` ×8                    | state に載るが**読み手0**                                             | 要る                   | 無い（同じ入力で同じ結果）       | ダメ                    |
