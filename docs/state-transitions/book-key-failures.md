...
`src-tauri/src/book/sfen.rs` の `book_key` が、局面の文字列を読む唯一の実装。
**この表が並べる G0〜G11 は全て `book_key` の中にある。**

公開している入口は2つで、どちらも `book_key` を呼び、失敗に種別と復帰操作を足すだけ。
...
**枝を足すときに更新する対象は `book_key`。** 入口の側に検査を足すと、片方の経路
...
- 不変条件 2: `a_long_token_is_truncated_in_the_reason` ✓。何をどう見ているかは
  そのテストの doc にある（ここへ写すと、片方だけ直したときに腐る）。
  **変異で確認: 断片を埋める5枝のどれか1つで打ち切りを外すと落ちる**
