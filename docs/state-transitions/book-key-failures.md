...
3ラウンド続けて出た**（R9 N-07 / R10 O-04 / R11 P-01 / R12 Q-02）。原因は枝そのものではなく、
...
| B トークン不足 | G2 / G3 / G4 ✓（`rejects_input_that_is_not_a_position`） | G0（B の理由文は出ない） |
...
| F 1トークンが長い | G7 / G8 / G10 に断片が入る ✓（`a_long_token_is_truncated_in_the_reason`） | G0 ✓ |
...
G7 / G8 / G10 の3つあるが、そこを通るテストが1本も無かった。
...
1. **合法な局面は必ず OK に着く。** 盤面は成駒（`+X` = 2字）と畳んでいない空きマス（`1` の並び）で
   最長 123 字、`position sfen ` の前置きと手数を足しても 160 字前後。`MAX_INPUT_CHARS` は
   これを下回ってはならない
...
- 不変条件 1: `a_maximally_spelled_board_is_accepted` ✓。駒箱の40枚を全て盤に置き、
  成れる34枚を `+X`、空きマス41を `1` の並びで書いた盤面（123字）を通す。
  **変異で確認: `MAX_INPUT_CHARS` を 128 に詰めると落ちる**
