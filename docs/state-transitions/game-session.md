...
| **E12** | **`stop` に応じない**          | `SearchOutcome::StopTimedOut`。`SEARCH_STOP_GRACE`（5秒）超過か、書き込みが `WRITE_TIMEOUT`（2秒）で返らない |
...
| 2   | 打ち切られたなら `stop` を送り、捨てる `bestmove` を `SEARCH_STOP_GRACE`（5秒）まで待つ |
