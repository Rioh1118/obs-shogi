...
**記号は他の表とも重なる。** `G0`〜`G2` は [game.md](game.md) の3つと完全に、
`A0`〜`A4` は [app.md](app.md)（上位の表）の5つと完全に重なる。**どちらも別物。**
**記号だけで書かず、どの表かを添えること**（`failure-surfacing.md`）。
...
※3 `accept_continue` が **`Err` で断る**検算は4つ。
**`moves` が「いまの写し＋決まった手」ちょうど1通りであること**（長さと接頭辞）、
末尾が直前に決まった手であること、`moves.len()` の偶奇が次の手番と合うこと、
**決まった1手**の形が通ること。

**手数の上限（`MAX_PLIES`）だけは断らない。** 4つを通ったうえで超えていたら、
`Ok` を返して `G2`（`Rule`）へ落とす。断ると、フロントが返せる列は一意に固定されて
いるのでやり直しても同じ `Err` になり、E15 で「アプリが裁定を返さなかった」として
畳まれる——返しているのに。**検算の後に置く**のは、食い違った長い列が接頭辞を
1手も見られないまま「上限に当たった」として終局しないため。

**権威はフロントだが、受け取ったものが直前の手の続きであることは確かめる。**
末尾と偶奇だけでは足りない——途中局面から始めた対局で「対局開始以降の手だけ」を
渡すと全部通り、写しが黙って短くなり、次の `go` は根に途中の手を継ぎ足した
**別局面**を送る。エンジンはそこの合法手を返し、フロントは現局面で非合法と裁定する。
...
**その間、対局は台帳から外れている。** `GameManager::close` は `sessions` から
外してから待つので、窓のあいだ `sessions` を引いても見つからない。
`closing` がその窓を埋めていて、`get_game_state` / `submit_game_move` は
「知らない対局」ではなく「いま閉じている」で断り、`list_games` はその ID を含める。
埋めないと、**生きている対局への再同期が「無い対局」として断られる**。
...
（`engine/child.rs` の `KILL_TIMEOUT`）、そちらを超えるとプロセスが残る。
...
`CLOSE_TIMEOUT` を超えても掃除が拾う。それでも残るもの（`SWEEP_TIMEOUT` が尽きて `kill` が
届かなかった後続、`KILL_TIMEOUT` を超えても OS が畳まなかった回、下に書く孫のエンジン）は
台帳の F-25 が持つ。
`kill` は `quit` を書かずにシグナルを送り、待ち手のタスクが終了状態を見届ける
（`engine/child.rs`）ので、stdin を閉じて走り続けるエンジンも落ちる。
**`#!` のラッパーから起こした孫のエンジンまで落ちるのは、unix で、`kill` の時点で子が
まだ生きている回だけ**（そのときはプロセスグループへ送る）。子（ラッパー）が先に終わって
いた回（`KillOutcome::AlreadyExited`）はグループへ送らず、非 unix（`.bat` / `.cmd` の
ラッパー）は子しか落とさない。どちらも孫を止めるのは stdin の EOF だけ（→ F-25）。
...
**`Over` の emit は `send_gameover` より先。** 後に回すと、終局からイベント到着まで
書き込み1件ぶん（`WRITE_TIMEOUT` ＋ 列の待ち）空き、その間フロントは減り続ける
時計を描く。**順序は未検証**——`send_gameover` の宛先が `UsiProtocol` の具象で、
観測する継ぎ目が無い（→ #377）。入れ替える変異でも落ちない。
...
※8 **手番でない側**の `info` を落とすのは `on_search_info` の `is_to_move` 1本だけ。
先読み中の側は手番ではないので、手番が変わった後に届いたものと同じ判定で落ちる。
**世代の照合は別にある**（`is_current_search`。→ ※13）——打ち切った探索が
取り消しと同時に吐いた `info` は、手番が合っていても別の局面のもの。

**`search.rs` では間引かない。** 探索タスクは起動時の値を握ったまま走るので、
`ponderhit` で本番へ昇格したことを観測できない。間引くと、先読みが当たった手番だけ
読み筋が1行も出なくなる（当たる率はエンジンが強いほど高い）。

**`is_to_move` の1行が守っているのは先読み中の側だけ。** 冗長と読んで消すと、
相手の手番中の先読みが画面に出る（世代は `info_from_a_stopped_search_is_not_shown`
が見る）。**間引きは1段も無い** → 埋まっていないセル。
...
3. `Running` が `SEARCH_GRACE` を超え、その間 `info` が1行も来ておらず、
   **かつそのエンジンがこの局で一度でも `info` を出したことがある**
   （`Player::has_spoken`）。
   **ここは持ち時間を見ない**——黙っているかどうかは持ち時間と無関係の信号で、
   足すと、持ち時間が長い対局で初手から固まったエンジンが持ち時間ぶん検出されない

**3 に `has_spoken` が要るのは、USI が `info` を義務にしていないから。**
1行も出さない実装（詰将棋ソルバを対局者に挿す、深さが変わったときだけ出すエンジン）を
沈黙で落とすと、正常に読んでいる `SEARCH_GRACE` 目に必ず負ける。
**引き換えに、一度も喋っていないエンジンを押さえるのは 2 だけになる**——
持ち時間が長い対局で初手から固まると、`持ち時間 + HARD_TURN_LIMIT` まで検出されない。
**2 は `has_spoken` を見ない。** そこが最後の歯止め。
...
時間切れも掛からず、探索タスクにも締切が無い）。3 から沈黙条件を外すと、
**一度でも喋ったエンジンは手番に入って `SEARCH_GRACE` 経った時点で必ず落ちる**
——正常に読んでいても、持ち時間が満額残っていても同じ（3 は持ち時間を見ない）。
...
※13 世代の照合は `Info` と `SearchOutcome` の両方に掛かるが、**踏めているのは `Info` 側だけ**
（`info_from_a_stopped_search_is_not_shown`）。`SearchOutcome` 側は「埋まっていないセル」を見ること。
...
**Rust の中で何箇所にあるかは `GameClocks::view` の doc**（数を2箇所に
書くと、片方だけ直る経路ができる）。
1つだけ変えると、画面に秒読みが残っているのに時間切れになる。

**期限は過去を指しうる。** 尽きた持ち時間は「尽きた時刻」のまま送られ、
同じ手の間は `ClockUpdated` を何度出しても動かない。0 でクランプして「いま」を
指す形にすると、期限が emit ごとに前へ進む一方で受け手の「いま」は毎秒しか
動かないので、**表示が `0:00` と `0:01` を往復する**（秒読み中はずっと）。
...
`running` が `null` になる理由は **`ClocksView::running` の doc** にある
（本文を2箇所に置くと、片方だけ直る経路ができる）。ここで押さえるのは1つ——
**`G1` と `G2` だけでなく `G0` でも `null` になりうる**こと。

**`G0` で `null` になるのは、渡す先に畳む探索があったときだけ。** 先読みが外れた、
...
`EngineRegistry::spawn` が `child::spawn` を直に呼んで実行ファイルを
