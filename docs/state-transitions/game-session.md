...
| `(G0, E13)` `info` の間引き                    | Rust は対局も解析も1行ごとに `emit` する。間引きは**受け手側**にあり、解析は `src/entities/analysis/model/useResultFlush.ts` が持つ（`RESULT_FLUSH_MS`）。対局は受け手そのものが無い。**`run_loop` は単一キューなので、`emit` が詰まると `bestmove` の処理がその後ろに並ぶ** |
