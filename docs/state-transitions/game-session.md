...
※4 `GameSession::close` は「止める → **畳まれるのを待つ**（`CLOSE_IDLE_TIMEOUT`
...
（`registry.rs` の `KILL_TIMEOUT`）、そちらを超えるとプロセスが残る。

エンジンへの書き込みは**1本の列**を通る（`protocol.rs` の `run_writer`）。
投入順がそのままワイヤ上の順になり、上限（`WRITE_TIMEOUT`）もそこ1箇所で
全ての `send_command` に掛かる。
...
ただし **`close_all` の呼び出し元は0**（`lib.rs` に終了時のフックが無い）。
アプリを閉じると対局中のエンジンがそのまま残る（→ 埋まっていないセル）。
...
3 は手が渡るたびに通る。`TurnChanged` は `running: null` で飛び、
動き出すのは次の `ClockUpdated` から。**`null` を「対局が止まった」と読まないこと。**
...
| `GameManager::close` の `Arc::try_unwrap` 失敗     | 中断だけ通して台帳へ戻り `Err` が返る（※4）。**戻ったセッションを閉じ直す呼び出し元が居ない**——`close_all` / `shutdown_all` に呼び出し元が0（`lib.rs` に終了時のフックが無い） |
