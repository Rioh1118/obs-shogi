...
`KILL_TIMEOUT` を超えたときと、**`kill` が `process.kill()` へ届かなかったとき**
（→ 台帳の F-25）。3つ目は上限ではない——`usi` の `kill` はシグナルの前に
`quit` を書くので、書き込みが折り返すとシグナルは一度も送られない。
stdin を閉じて走り続けるエンジンがこれに当たる。**`quit` で自発的に終わった
エンジンも同じ EPIPE を返す**ので、区別は `classify_kill_failure`
（`protocol.rs`）が stdout の EOF と併せて付ける。付けないと、正常に閉じた回に
毎回「残っているかもしれない」と記録される。**付けた後も推定のまま**——
`usi` の `UsiEngineHandler` は子プロセスの handle を私有していて、
終了状態を問い合わせる口を公開していない。確かめられるのは
両方のパイプが閉じていることまで。
