...
**登録は4つとも同じ。** 違うのは警告を出すかどうかと、局面が入るかどうか。

**2行目は CSA だけ。** `csa` クレートが読み残しを捨てて `Ok` を返すので、
途中で切れた CSA を読み手が見つけて `warns` に積む（`kifu_reader.rs` の
`warn_if_moves_were_dropped`）。**断らないのは、断るとそのファイルの局面が
1件も入らなくなるから** — 1手落ちただけで全部を捨てる代償に見合わない。
数え方はクレートの文法を写していないので外れることがあるが、
出口が警告なので外れても失われるものが無い。
