...
| ------------------------------------ | ------------------------------ | -------- | --------------------- | -------------------- | --------- |
| `Ok`（`warns` 空）                   | 読めた（読み残しも見つからず） | 出さない | する                  | 入る（⚠️ 下記）      | **真**    |
| `Ok`（`warns` あり）                 | 読めたが一部を採れなかった     | **出す** | する                  | **採れたぶんは入る** | **真**    |
| `Err(ParseFailed)`                   | 読めなかった                   | **出す** | **する**              | 入らない             | 偽        |
| `Ok(NothingToIndex)`（`warns` 空）   | 空に見え、読み残しも見つからず | 出さない | **する**              | 入らない             | **真**    |
| `Ok(NothingToIndex)`（`warns` あり） | 空に見えるが、読めなかったせい | **出す** | **する**              | 入らない             | 偽        |

**登録は5つとも同じ。** 違うのは警告・局面・`indexed` の3つ。

**`indexed` は「局面が入ったか」ではない。** 下から2行目——このアプリが新規作成した
棋譜——は局面が入らないのに**真**。中身が無いのが正しい姿なので、失敗に数えると
**棋譜を1つ作るたびにワークスペースが黄色くなる**。
...
**それでも検査が黙る形は真に落ちる**（上の2通り）。UTF-16 の CSA が空に見える回は
「本当に空」と数えられる。
...
ならず0件と数える。どちらも
**警告の欄が空の行**（`Ok` で `warns` 空、`NothingToIndex` で `warns` 空）に落ちるので、
...
| 本体のファイルが消えた／開けない       | `restore-ok` → **U** | **何も出ない**（`.bak` から1世代古いものが載る） |
