...
| ----------------------------------- | ------------------------------ | -------- | --------------------- | -------------------- | --------- |
| `Ok(Indexable { warns: [] })`       | 読めた（読み残しも見つからず） | 出さない | する                  | 入る（⚠️ 下記）      | **真**    |
| `Ok(Indexable { warns: [_] })`      | 読めたが一部を採れなかった     | **出す** | する                  | **採れたぶんは入る** | **真**    |
| `Err(ParseFailed)`                  | 読めなかった                   | **出す** | **する**              | 入らない             | 偽        |
| `Ok(NothingToIndex { warns: [] })`  | 空に見え、読み残しも見つからず | 出さない | **する**              | 入らない             | **真**    |
| `Ok(NothingToIndex { warns: [_] })` | 空に見えるが、読めなかったせい | **出す** | **する**              | 入らない             | 偽        |
...
| 版が古い（`CACHE_VERSION` 違い）       | `restore-ng` → **B** | 何も出ない                  |
