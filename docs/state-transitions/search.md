...
警告は打ち切った線を名指す（`to_user_message` が「本譜の」「N手目から分かれた
...
- **読めなかった理由をどこで警告にするかが違う。** `api` は `warns` に混ぜて
...
`src-tauri/tests/` にあるのは規約の検査（ソースを文字列として読むもの）と、
上限どうしの関係を式で固定する `engine_timeouts.rs` だけで、
状態機械そのものを回すテストは無い。
