...
- `/review` — reviewer を並列で走らせ、所見を「機械にする / 直す / 送る」に振り分けて処理し切る

**レビューは2巡で終わる。所見が尽きるまで回さない。** 尽きない——実測の最長は**46ラウンド**で、
うち13ラウンドは reviewer 自身が「新しく挙がったものは無い」と書きながら続いていた。
成果は**直した所見の数ではなく、増えた機械の数**で数える。

**機械化できる所見は、機械にしてから閉じる。** 個別に直すだけだと、同じ穴が次のラウンドで
別の綴りで再出現する。入れなかったせいで2〜17回再提案された項目が
`.claude/knowledge/mechanization-backlog.md` に並んでいる。**two-strikes を数えるのはこの表。**
「1件目だから足さない」を、表を見ずに書かない。

**画面に触る前に `docs/spec/` を読む。** 画面ごとに「いま何ができて、何ができないか」が
書いてある。触った画面の仕様が現物と違うようになったら、同じ PR で直す。

**新しい UI を書く前に `docs/spec/design-language.md` を読む。** 段の切り方・色の選び方・
ホバーの向き・重なりの順は既に決まっている。**その場で決め直さない。**
...
- **文脈を前提にしない。そのコメントだけを読んで意味が通ること。**
  読み手は issue も会話も差分も見ていない。次は全て「読めない」側:
  「上と同じ理由で」（どの理由か書いていない）／「#420 の対応」（issue を開かないと読めない）／
  「例のケース」「先ほどの」／「ここでは特別に」（何に対して特別かが無い）。
  **指すなら綴りで指す。** 条件なら条件式を、型なら型名を、場所なら関数名を書く
- **「〜だから」と書いたら、その条件式がコードのどの行かを指せること。**
  指せないなら、その理由はもう実装と合っていない
