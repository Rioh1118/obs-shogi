...
//! （下の層だけを組んで回す、ができない）。
//!
//! ここで見るもの。**本数は書かない**（検査が増えるたびに嘘になる。ADR-0008 決定1）。
//!
//! 1. モジュール間に環が無いこと
...
//!
//! ほかに、外から `engine/` へ入る辺と `book/` から出る辺が控え
//! （`ENGINE_ENTRIES_FROM_OUTSIDE` / `BOOK_OUTWARD_BRANCHES`）と一致すること、
//! `engine/mod.rs` が宣言だけであること、全モジュールが `LAYERS` に載ること、
//! `may_use` に実体の無い辺が無いことを見る。
//!
//! ## 走査の限界
//!
...
/// 段3から段8へ届く経路が、`graph()` を通るどの検査にも掛からずに通る。
///
