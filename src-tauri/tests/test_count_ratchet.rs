...
//!
//! **テストは消しても緑になる。** #91 のレビュー ラウンド3で、パーサの上限・
//! 切れ検出・`# NOE:0` の迂回を守っていたテスト5本とコンパイル時 assert 2本が
//! 消えているのが見つかった。消したのは編集スクリプトのスパン置換で、
//! 同じコミットで別のテストを足していたので**本数が減らず気づけなかった**。
//! 消えた後もガードのコードは残っていたが、それを外す変更が緑で通る状態だった。
//!
//! カバレッジでは代わりにならない。カバレッジが落ちるのは「コードが実行されなく
//! なったとき」で、ここで起きたのは「守るテストだけが消えたとき」。
//!
...
///
/// 例えば 142 のまま 200 本まで増やすと、57 本消しても緑で通る。
#[test]
