...
//!
//! `startGame` が返すのはフラットな文字列で、フロントはその中身だけで
//! 「設定が誤っている（直させる）」と「遅かっただけ（再試行させる）」を分ける
//! （→ `docs/state-transitions/failure-surfacing.md` の F-27）。
//! 分ける鍵は `TIMED_OUT` の綴り1つしかない。
//!
...
//! 変数を1つ挟めば（`let m = "…"; EngineError::Timeout(m)`）素通りする。
//! 時間切れを `Timeout` 以外のバリアントで返す形も見ていない。
...
/// 時間切れを作る式が、必ず目印を含むこと。
///
/// **含まないと「設定の誤り」に分類される。** `startGame` の TSDoc が
/// そう書いていて、フロントはそれを契約として読む。プロセスは正常に起き、
/// パスも設定も正しいのに「エンジンのパスを直せ」と案内される——
