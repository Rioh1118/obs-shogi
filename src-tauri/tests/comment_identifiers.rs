...
//!
//! `src/__tests__/docsIdentifiers.ts` の Rust 版。あちらは doc と `src/**` の TS コメントを見て、
...
///
/// **ここは Rust のコメントだけを見る。** 免除のリストは3つあり、それぞれの
/// 掛かる範囲は `src/__tests__/docsIdentifiers.ts` の `EXEMPT` の doc が持つ
/// （そちらは1つで2つの範囲を裁くので、足すときの注意もあちらに在る）。
/// **同じ綴りを2つ以上へ足すことがある**（`line_buffer` はここと `docsIdentifiers`）。
/// 片方にしか要らない綴りも在る（`peek_text` はここだけ —— 表の中で
/// バッククォートに囲まれていないので `docsIdentifiers` は拾わない）。
const EXEMPT: &[&str] = &[
    // --- `std` ---
    // 確保に失敗したときの挙動を説明している
    "handle_alloc_error",
    ...
    "stopped_at",
    // --- OS ---
    // パスの上限を「弾くための値ではない」と言うときの比較対象
    "PATH_MAX",
    // --- やねうら王（C++。定跡の形式の出典） ---
    // 欄をタブで区切った定跡を本家が読めることの根拠
    "peek_text",
    // 末尾の改行を要求しないことの根拠
    "line_buffer",
];
