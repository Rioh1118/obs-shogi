...
///
/// **これは「壊れた棋譜」の印ではない。** 画面に開くほうの `tsshogi` は
/// どの行パターンにも当たらない行を読み飛ばすので、同じファイルを最後まで読む。
/// つまりこの警告が指しているのは**索引側だけが読めない範囲**で、
/// 整形の届く先が伸びれば減る。整形で指し手でない行まで捨てないのは、
/// 捨てると**本当に指し手が欠けている棋譜まで黙って通る**から。
///
/// # 数え方は当てにいかない
///
