...
pub struct IndexWarnPayload {
    ...
    ///
    /// **描かれるのは新しい順の5件。** `WorkspaceTab` は `pickWarns` を通し、
    /// あちらは受け取った列を逆順にしてから、場所の警告 → `path` が空の場所 →
    /// 棋譜1件の警告、の順に並べて先頭から取る。**新着は必ず前に来る**ので、
    /// 後から出した警告が読めないという状態にはならない（#465 はこの経路で解いてある）。
    ///
    /// **溜め方は種類ごとに枠がある。** `reducer` の `appendWarn` は同じ鍵の警告を
    /// 末尾へ動かして重複を除き、`WARNS_KEPT`（200）を超えたら場所を
    /// `PLACE_KEPT`（20）だけ残して残りを棋譜1件の枠に充てる。
    /// **場所の警告が棋譜の警告に押し流されない**のはこの枠のため。
    ///
