...
//! 1つの桶に何本も溜まって二分探索が本数ぶん走る。畳むと1本になる。
//!
//! **持っているのは畳み方（合流の順序と生存判定）だけ。いつ畳むかは呼び手が決める。**
//!
//! | 呼び手 | いつ |
//! | --- | --- |
//! | `store/snapshot.rs` の `with_files` | 積み増した桶が [`COMPACT_THRESHOLD`] を超えたとき |
//! | `cache/format.rs` の `compact_all_buckets` | チェックポイントを書くたび、**全桶**。選ぶ余地が無い —— blob は桶ごとに1本の昇順で、崩れていれば `encode_all` が書くのを拒む |
