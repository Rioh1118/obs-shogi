...
//!
//! 索引は1ファイルにつき1つのセグメントを積む。桶が無いと、引くたびに
//! **全ファイルぶんのセグメントを舐める**ことになる。鍵の上位8ビットで
//! 先に振り分けると、舐める本数も各セグメントの長さも 1/256 になる。
//!
//! 圧縮の判断（`store/index_store.rs` の `COMPACT_THRESHOLD`）を桶ごとに
//! できるのも桶で分けているから。
//!
