...
//!
//! 桶で分けて得ているのは2つ。**1つの桶が持つ項目が索引全体の 1/256 になる**ことと、
//! **圧縮の判断を桶ごとにできる**こと（`store/index_store.rs` の `COMPACT_THRESHOLD`）。
//!
//! **セグメントの本数は減らない。** 積む単位は「1ファイル × 1桶」なので
//! （`index_store::insert_many_file_segments`）、大きいファイルはほぼ全ての桶に
//! 1本ずつ積む。
//!
