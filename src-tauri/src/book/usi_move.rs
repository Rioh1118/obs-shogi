...
///
/// **範囲の検査をここで済ませる。** `Square::new` も範囲外を `None` にするが、
/// 引き算を先にすると `b'0'` を下回った差が `u8` で巻き上がり、範囲内の別の升に
/// 化ける（`'/'` は `b'0' - 1` なので筋 255 ではなく 0 ではない値になる）。
...
mod tests {
    ...
    /// 升として成立しない綴りは読めないこと。
    ///
    /// **`0` と `/` を並べてあるのは引き算の巻き上がりを見るため。**
    /// `file - b'0'` を先に書くと `/`（`b'0' - 1`）が 255 になり、範囲検査が
    /// 「大きすぎる」で弾くので、この2つは同じ理由では落ちない。
    #[test]
