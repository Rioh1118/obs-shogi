...
/// 替えるなら、次を同時に片付けること。
///
/// - **揃える先は「最初の出現位置に、後の値」。** 本家の `b = bp` は既存の要素を
///   その場で置換するので、読み込んだ後の並びもそうなる。
...
///   どうするかを決める必要がある。**本家の答えは入力で分かれる** —— 欄が無ければ `1`
///   （`book.cpp:94` の `u64 move_count = 1`）、`none` や数値でない綴りなら `0`
///   （`misc.cpp:1677-1680` の `get_number` が `atoll` へ落とす）。
///   **こちらは [`optional_number`] がその全部を `None` へ潰しているので、
///   本家の規則をそのままは当てられない。** 区別を残すかどうかから決めること
///   （`types.rs` の `count` の doc も同時に直す対象）
