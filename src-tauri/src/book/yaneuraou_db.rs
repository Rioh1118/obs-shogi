...
///
/// **合わせない。** 重複は同じ定跡ファイルの中の書き損じで、どちらを残しても
/// 同程度に恣意的（本家も「どちらが正しいか」を決めてはいない。`overwrite` の
/// 既定がそうなっているだけ）。**どちらでもよいので、畳む側が1パスで済む方を採る** ——
/// [`keep_first_of_each_move`] の2つの枝はどちらも前から1回なめるだけで、
/// 後勝ちにすると「先に入れた要素をどこへ上書きするか」を覚える必要が出る。
///
/// **順序への依存は理由にならない。** 先勝ちも同じだけ依存する（上の例の2行を
/// 入れ替えれば、先勝ちでも返るのは 900 になる）。
///
