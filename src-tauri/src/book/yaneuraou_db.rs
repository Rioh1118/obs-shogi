...
fn parse_limited<R: BufRead>(
    ...
    //
    // - 拒否の側: 本家は末尾の改行を要求しない（`misc.cpp:1133-1140` の
    //   `TextReader::read_line_simple`。EOF でも `line_buffer` が空でなければ
    //   1行として返す）。改行が無いだけの完全な定跡を
...
fn flush(
    ...
    match positions.entry(key) {
        ...
        // `... b - 1` と `... b - 31` は同じキーになる（本家も手数違いの重複を
        // 書き出す側で畳んでいる。`book.cpp:406-409`）。
        //
...
///
/// **後勝ちへ替えてよい。** 費用は変わらず（どちらの枝も1周のまま）、順序への依存も
/// 先勝ちと同じ（上の2行を入れ替えれば、先勝ちでも返るのは 900 になる）。
/// 替えるなら、次の3つを同時に片付けること。
///
/// - [`keep_first_of_each_move`] の**2つの枝が、残す位置まで一致すること。**
///   走査の枝は `any` を `position` にして `swap(j, i)` すれば最初の出現位置に残るが、
///   `HashSet` の枝を逆順に回すと最後の出現位置に残る。**[`SCAN_LIMIT`] を境に
///   「先頭がその局面の best move」に別の手が座る**
/// - `HashSet` の枝は、**重複が交互に出る長い列**でしか観測できない。いまその形の
///   テストは無く、先勝ちを固定している2本はどちらも走査の枝しか通らない
/// - 採択回数の合算は [`BookMove`] の `count` が `Option<u64>` なので、`None` を含む組を
///   どうするかを決める必要がある。`None` を 0 と読むと、採択回数を書いていない定跡が
///   「0回選ばれた」として順位付けに載る
///
...
mod tests {
    ...
    /// v1.20.0 までの ShogiHome は score と depth を省くとき空文字を書き出す。
    /// **その定跡はいまも配布されている**ので、連続した空白を畳むと欄が1つずつ
    /// ずれ、深さが評価値として画面に出る（現行の綴りは一次資料の表を見ること）。
    #[test]
    ...
    /// 「長すぎる」に落ち、引用の経路を1バイトも通らない。落ちてくる文面は
    /// それでも短いので、長さだけを見る assert は素通りする。
    #[test]
    ...
    /// 表の (S0, E6) / (S0, E7)。局面より先に来た指し手は落ちる。
    ///
    /// **診断は見出しの有無で変えない。** 見出しは要求しないので、先頭の局面行を
    ...
    /// **どちらの上限にも2倍の余裕を要求する。** 実物に近い値を置くと、版が
    /// 重なった時点で実利用者が弾かれ、そのとき出せる復帰操作が無い。
    ///
    /// 単価が実測と合っているかは機械では守れないので、**この2本が唯一の歯止め**。
    /// 消すと、上限を実物へ近づける変更に何の抵抗も無くなる。
    ...
    fn the_limit_message_names_how_far_it_got_not_the_limit() {
        ...
        // 渡した上限そのものを出していないことを見る。定数と突き合わせると、
        // 上限を小さくしたテストでは何も検出できない。
        assert!(
