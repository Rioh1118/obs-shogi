...
///
/// **合わせていない。これは理由ではなく差の記録。** 重複は同じ定跡ファイルの中の
/// 書き損じで、どちらを残しても同程度に恣意的。
///
/// 使えない理由を2つ潰してある。**費用の差は無い** —— [`keep_first_of_each_move`] の
/// 走査の枝は `any` を `position` へ替えれば1パスのまま後勝ちになり、`HashSet` の枝も
/// `rev()` で足りる。**順序への依存も理由にならない** —— 先勝ちも同じだけ依存する
/// （上の例の2行を入れ替えれば、先勝ちでも返るのは 900 になる）。
///
/// 変えるなら、[`keep_first_of_each_move`] の2つの枝と、先勝ちを固定しているテストを
/// 同時に変えること。**採択回数を合算する本家に本当に合わせるなら、こちらの
/// [`BookMove`] にその口が無いことも決める必要がある。**
///
