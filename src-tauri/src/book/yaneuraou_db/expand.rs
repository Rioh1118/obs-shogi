//! 1行ずつ読んで、局面ごとの候補手へ畳む。
//!
//! この形式はメモリへ全部載せるので、走査そのものが上限との競争になる ——
//! 局面を跨ぐたびに見積もりを更新し、超えたらそこで断る（[`super::limits`]）。
//!
//! **溜めるのは1局面ぶんだけ。** 局面が変わった時点で map へ渡し、
//! 同じ局面が後からもう一度現れたら結合する。全部読んでから畳むと、
//! 重複を抱えたままの列が読み切るまで残る。
...
impl crate::book::reader::BookReader for YaneuraouDbReader {
...
pub(crate) fn load(path: &Path, size: u64) -> Result<YaneuraouDbReader, BookError> {
    ...
    // ファイルサイズの上限（[`super::limits::MAX_FILE_BYTES`]）はここでは見ない。形式の分岐より
...
#[cfg(test)]
mod tests {
    use crate::book::yaneuraou_db::limits::MAX_FILE_BYTES;
    ...
    /// （[`super::limits::MAX_FILE_BYTES`]）は展開の上限より小さいので、その2つを並べると
    ...
    fn tolerates_a_utf8_bom() {
        let dir = test_support::dir::temp_dir("book-bom");
