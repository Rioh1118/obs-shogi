...
//! （その時点でスワップに入っており、未保存の棋譜ごと落ちる）ので、先に断る。
//!
//! - [`MAX_FILE_BYTES`] —— ファイルの大きさ。**粗い前段**で、
//!   メモリの上界はここではなく展開後の見積もりが持つ
//! - [`MAX_LINE_BYTES`] —— 1行の長さ。展開の見積もりは行を受理した**後**にしか
//!   走らないので、行そのものの長さはその勘定に入らない
//! - [`MAX_EXPANDED_BYTES`] —— 読みながら数えた局面数・候補手数からの見積もり
//!
//! **どれが欠けても穴が空く。**
//!
