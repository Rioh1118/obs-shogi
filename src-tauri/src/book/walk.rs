...
pub(crate) fn walk_lines(
    ...
    let (position, side) = start_position(start)?;

    moves
        .iter()
        .map(|usi| walk_line(book, &position, side, usi))
        .collect()
}
...
fn start_position(key: &BookKey) -> Result<(PartialPosition, Color), BookError> {
    let side = side_in_key(key)?;
...
/// キーの手番。
///
/// **`PartialPosition` から手番を取り出す口が無い**（`side` は private で getter も
/// 無い）ので、綴りの側から読む。[`to_book_key`] が `b` / `w` 以外を弾いた後なので
/// ここへ来る綴りは必ずどちらか —— それでも `Err` を持つのは、キーの組み立てが
/// 変わったときに黙って先手固定にしないため。
fn side_in_key(key: &BookKey) -> Result<Color, BookError> {
    match key.as_str().split_whitespace().nth(1) {
        Some("b") => Ok(Color::Black),
        Some("w") => Ok(Color::White),
        _ => Err(BookError::new(
            BookErrorCode::InvalidSfen,
            format!(
                "定跡のキーから手番を読めない（{}）。不具合として報告すること",
                excerpt(key.as_str())
            ),
        )),
    }
...
fn walk_line(
    ...
    // 手番を別に持つのは打つ手のため。盤上の手は `make_move` が局面の側の手番を使うので、
    // ここがずれても動いてしまい、**打つ手だけが「自分の駒ではない」で弾かれる**
    let mut side = start_side;
    let mut next = first.to_string();
    let mut plies = 0;

    loop {
        let Some(mv) = to_core_move(&next, side) else {
        ...
        let key = to_book_key(&position.to_sfen_owned())?;
...
mod tests {
    ...
    fn key_after(moves: &[&str]) -> String {
        let (mut position, mut side) = start_position(&key(HIRATE)).expect("平手は読める");
        for usi in moves {
            let mv = to_core_move(usi, side).expect("綴りは正しい");
    ...
    /// [`side_in_key`] を取り違えると `make_move` が「自分の駒ではない」で弾き、
