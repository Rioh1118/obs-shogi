...
pub struct BookInfo {
    ...
    /// 開いた時点で確定する。
    ///
    /// **意味は「`lookup` が引きうるキーの数」。** 正規化して重複を畳んだ後の数で、
    /// ファイルの申告値（やねうら王の `# NOE:N` など）ではない。申告値で埋めると、
    /// 同じファイルが reader の実装によって別の数を出す（申告は正規化前・重複込み）。
    /// #96 で複数の定跡を並べたとき、この数字で厚さを比べられなくなる。
    ///
    /// 申告値の使い道は、**ファイルから読んだ局面行の本数**と突き合わせて途中で
    /// 切れたファイルを見つけること。この数（正規化・重複畳み込み後）と
    /// 比べてはいけない。手数違いの重複がある正常な定跡が拒否される。
    ///
    /// **数えられない形式は `null`。** 0 は「本当に0局面」を意味する。
    /// 同じ値にすると、局面数の書かれていない定跡が「空の定跡」として表示される
    pub position_count: Option<u64>,
    /// 読めずに捨てた欄の数。**0 でないなら、候補手の `value` / `depth` /
    /// `count` の `null` は「もともと無い」ではなく「読み損ねた」かもしれない。**
    ///
    /// 欄がずれた定跡（列を1つ落とした自作定跡など）を開くと、
    /// `optional_number` が `Err` を `None` に潰して開くこと自体は成功する
    /// （1つの綴りのために数百 MB を捨てない、という判断）。
    /// **その事実がここに出ないと、利用者に区別する材料が1つも無い。**
    ///
    /// **数えられない形式は `null`。**
    pub dropped_fields: Option<u64>,
}
