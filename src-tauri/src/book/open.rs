...
//! 指定と実体の形式の一致 → ファイルであること → reader を持っている形式か →
//! 形式ごとの大きさの上限 → 中身、の順に落ちる。後ろの4つの実装は
//! [`crate::book::formats::open_reader`] にある（`metadata` を1度しか取らないため）。
//! **未対応が大きさより前にある理由**は、そちらの `support` の分岐に書いてある。
//!
...
mod validated {
    ...
    ///
    /// 保証するのは3つだけ。**空でない / NUL を含まない / 絶対パス。**
    /// 実在は見ていない。長さの上限も掛けていない（深い階層に定跡を置く利用者を
    /// 弾かないため。長いパスの打ち切りは、エラーへ載せるときにだけ効く）。
    ///
    ...
    pub(crate) fn validate_book_path(raw: &str) -> Result<ValidatedBookPath, BookError> {
        ...
        // 空のパスは canonicalize が NotFound にする。「見つからない」と言われた
        // 利用者は探し直すが、探す対象が無い。
...
mod tests {
    ...
    ///
    /// `validate_book_path` は長さで弾かないので、**検査を通る長いパスは
