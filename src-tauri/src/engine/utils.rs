...
///
/// **`search/` は別の規約を持つ**（`search::message::for_screen`）。あちらは改行を通し
/// （棋譜の引用を読める形で残すため）、上限を固定で持つ。**同じ判断ではないので
/// 片方だけ読んで揃えないこと。** 実装を1本に寄せるには `tests/layering.rs` の
/// 規則3（`engine/` は crate の他の枝を `use` しない）から動かすことになる。
///
