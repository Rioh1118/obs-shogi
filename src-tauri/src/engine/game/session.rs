...
//! **フロントにある**（`src/entities/game/lib/gameOutcome.ts`）。
//! 持将棋の27点法・24点法はそこに入らない——宣言の規則なので
//! `src/entities/game/lib/jishogiDeclaration.ts` が別に持つ。
//! この層はどれも持たないので、指した後の局面が終局かどうかを自分では知れない。
//!
...
/// それだけで「再試行してよい失敗」を見分ける（→ 台帳の F-27）。
...
/// **腕は2本あり、見ている信号が違う。**
///
/// 1本目（`budget_ms` ＋ `HARD_TURN_LIMIT`）は持ち時間を見る。**持ち時間を
/// 過ぎたことだけでは落とさない**——`enforce_engine_timeout` が偽のまま持ち時間が
/// 尽きると `budget_ms` は 0 に張り付くので、持ち時間だけを条件にすると
/// **そこから即座に効き始める**。利用者は「時間切れを成立させない」と指定したのに、
/// 時計が尽きた瞬間から `HARD_TURN_LIMIT` 後に必ず負けることになる。
///
/// 2本目（`silent_for`）は**持ち時間を1つも見ない**。黙っているかどうかは、
/// 持ち時間が残っているかとは別の信号。
///
...
/// 見積もりを散文だけに置くと、番人の上限を式で書くときに数え直す口が無い。
...
mod tests {
    ...
    ///
    /// ここが持つのは、手番でない側が本番の思考をしている（`A1`）、
    /// 前に止めた分がまだ返っていない（`A3`）、何も走っていない（`A0`）の3腕。
    /// 先読みの当たり／外れは `a_ponder_that_missed_is_stopped_and_restarted`、
    /// 止めたのに応答しない側は
    /// `handing_the_turn_to_an_unresponsive_engine_ends_the_game_and_stops_there` が持つ。
    ///
