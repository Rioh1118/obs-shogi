...
impl SideClock {
    ...
    /// 初手の使える時間が 0 になり、**対局開始から最初の tick
    /// （`session.rs` の `TICK` 1つぶん）で時間切れ負け**になる。
    ///
...
impl GameClocks {
    ...
    pub fn view(&self, running: Option<(Side, u64)>, now_epoch_ms: u64) -> ClocksView {
        let running = running.map(|(side, elapsed_ms)| {
            ...
            // 期限は `CLOCK_EMIT_INTERVAL` ごとに前へ進む。受け手は秒単位へ切り上げて描き、
            // 「いま」はそれより粗い間隔でしか更新しないので、`main_zero_at - now` が
            // 0 と1秒弱を往復し、**表示が `0:00` と `0:01` を交互に出す。**
...
mod tests {
    ...
    ///
    /// 積まないと `budget_ms` が 0 になり、`on_tick` の2回目（`TICK` 1つぶん後）で
