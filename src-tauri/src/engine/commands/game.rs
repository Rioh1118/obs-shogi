...
struct TauriEvents {
    ...
    /// 返らなかった場合も、終局の1行に `RulingTimeout` と理由が残る（→ 台帳の F-19）。
...
/// `rulingTimeout` になり、ログにその `game_id` が一度も出ない。
///
...
mod tests {
    ...
    ///
    /// 食うと、B局は `RULING_TIMEOUT` で `rulingTimeout` になるのに、
    ...
    /// 残らない——`RULING_TIMEOUT` で `rulingTimeout` になった B局を、ログから追えない。
    ///
