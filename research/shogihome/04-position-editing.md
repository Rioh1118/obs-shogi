...
`specs/position-editing-mode.md` は 44 行の短い文書で、次を決めている。
...
| 論点           | ShogiHome                                      | obs-shogi の計画（2026-06・追跡外）              | obs-shogi の 2026-09 時点の案（**未文書化・出典なし**） |
| -------------- | ---------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------- |
...
3列目は口頭で出た案で、**リポジトリのどこにも記録が無い**。
決めるときは `docs/OPEN-QUESTIONS.md` に起こすこと。

ShogiHome は「初期局面を変える＝この棋譜を作り直す」で、
...
- **「余った駒をどこへ置くか」に、3つの別々の答えがある。**
  ...
  2026-09 の案はその中間で、「駒箱 ＋ 一時置き場」を2つに分ける。これは
  **「一時的に外す」と「その駒を局面から消す」を区別できる**利点がある一方、
  画面資源を1ブロック余計に食う。
- 「移動先に駒があったら駒台へ移す（成駒は不成に戻す、玉だけ入れ替え）」は
  **仕様として書かないと必ず抜ける類の規則**。obs-shogi 側の
  obs-shogi 側で未定義のまま残っているのが、まさにここ。
- **`specs/` に仕様書が1本ある**ことの効き目が大きい。44 行で、
