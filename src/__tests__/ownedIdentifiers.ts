 ...
 *
 * **`docsIdentifiers` との違いは、綴りをどこに探すか。** あちらは repo 全体に
 * 1度でも現れれば緑にする（あちらの doc の「限界」1番）。ここは**所有者の側にだけ**
 * 探す。`isActive` のように、別の部品が同じ綴りを別の意味で持っている改名跡は、
 * あちらでは原理的に拾えない —— **この検査が捕まえたいものはほぼ全部そこに落ちる。**
 *
 * 拾う綴りの形（camelCase を受けるか）と見る範囲（`docs/**` と `src/**` の
 * コメント）は、**あちらと同じ**。違いだと書かないこと。
 *
