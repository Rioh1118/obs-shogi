 ...
 *
 * **括弧の形は、台帳と綴りが割れる2つだけを見る** —— 接頭辞2文字のものと、
 * ハイフンの後ろに英字が付くもの。`.claude/reviews/` はこの2形で採番している。
 *
 * **1文字の接頭辞＋数字は見ない。** `failure-surfacing.md` の失敗の台帳と綴りが重なり、
 * そちらは**現に生きている参照**でコメントから20箇所以上が引いている。
 * 見ると、正しい参照を「指す先が消えた」として赤くする。算術の `N-1` も巻き込む。
 *
 * 形は正規表現そのものを読むこと。ここに例を書くと自分で落ちる
 */
const REVIEW_TAG = /^\s*\/\/\s*\d+:|\([A-Z]{2}-\d+\)|\([A-Z]-[A-Z]\d+\)|→\s*r\d+|\br\d+\s*→\s*r\d+/;
