 ...
 * `.claude/reviews/` の採番はこの3形で9割を占める。
 *
 * **1文字の接頭辞＋数字だけは見ない。** `failure-surfacing.md` の失敗の台帳と
 * 綴りが重なり、そちらは**現に生きている参照**でコメントから25箇所が引いている。
...
describe("コメント", () => {
  it("変更の経緯を書いていない", () => {
    ...
    //
    // **床は実測から1割ほど下に置く。** 近づけすぎると、コメントを数本消しただけで
    // 「読めていない」と名乗って止まる —— 原因を誤って指す赤は、無い赤より悪い。
    // 遠すぎてもいけない: 字下げされた行頭 `//` を `block` の欄へ流し込む変異は
    // `line` を6割動かすが、床が実測の1割なら通る。
    // **実測値をここに書かない** —— 上の `text.length > 2` を通した後の数なので、
    // 生の一致数と取り違えると、その数へ床を上げた人が違反0のまま赤くする。
    // 数えたいときは3つの変数を一時的に出力すること。
    //
    // **床では取れない族がある。** どちらも欄の数をほとんど動かさないので、
    // 注入でしか見つからない:
