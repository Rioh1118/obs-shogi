 ...
 *
 * **括弧の形は接頭辞2文字だけを見る。** 1文字の接頭辞は
 * `failure-surfacing.md` の失敗の台帳と綴りが重なり、そちらは**現に生きている参照**で
 * コメントから20箇所以上が引いている。1文字まで見ると、正しい参照を
 * 「指す先が消えた」として赤くする。算術の `N-1` も巻き込む。
 * いまレビューが振っているのは全て2文字なので、これで取りこぼさない。
 *
 * 形は正規表現そのものを読むこと。ここに例を書くと自分で落ちる
 */
const REVIEW_TAG = /^\s*\/\/\s*\d+:|\([A-Z]{2}-\d+\)|→\s*r\d+|\br\d+\s*→\s*r\d+/;
