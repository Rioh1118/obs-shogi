 ...
 */
const REVIEW_TAG = /^\s*\/\/\s*\d+:|\([A-Z]{1,2}-\d+\)|→\s*r\d+|\br\d+\s*→\s*r\d+/;
...
describe("コメント", () => {
  it("変更の経緯を書いていない", () => {
    ...
    //
    // **床は実測のすぐ下に置く。** 大きく下に置くと、形を1種類まるごと失っても越える ——
    // 行末の `//` だけを読まなくなる変異は `line` を6割落とすが、床が実測の1割なら通る。
    // **実測値をここに書かない** —— 上の `text.length > 2` を通した後の数なので、
    // 生の一致数と取り違えると、その数へ床を上げた人が違反0のまま赤くする。
    // 数えたいときは3つの変数を一時的に出力すること。
    //
    // **床では取れない族がある** —— `#` を行頭だけ・空白付きだけに絞る変異は
    // `hash` をほとんど減らさない。そちらは注入でしか見つからない
    expect(read.hash, "`.sh` から `#` コメントを読めていない").toBeGreaterThan(190);
