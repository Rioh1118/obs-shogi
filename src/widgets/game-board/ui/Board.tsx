...
function Board() {
  const { state, derived, helpers, selectSquare, clearSelection } = useGame();

  const shogi = derived.player?.shogi;
  const selectedPosition = state.selectedPosition;
  const legalMoves = derived.legalMoves;
  ...
  useEffect(() => {
    if (promotionState && shogi) {
      const currentMoveCount = derived.player?.tesuu || 0;
  ...
  }, [promotionState, derived.player?.tesuu, shogi, clearSelection]);
  ...
  const handleSquareClick = async (x: number, y: number) => {
    ...
    if (selectedPosition?.type === "square") {
      ...
      if (targetMove) {
        ...
        // 成り判定
        const canPromote = helpers.canPromoteMove(derived.player, shogiMove);
        const mustPromote = helpers.mustPromoteMove(derived.player, shogiMove);
        ...
        if (mustPromote) {
          ...
          promote = await new Promise((resolve) => {
            setPromotionState({
              ...
              color: fromPiece.color,
              moveCount: derived.player?.tesuu || 0,
              resolve,
            });
          });
        }
