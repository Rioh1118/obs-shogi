...
describe("ツリーが開いた棋譜を盤に載せる橋", () => {
  ...
  /**
   * **そのファイルを諦めさせない。** 落ちるのはその時点の中身のせいで、外で直せば載る
   * （台帳の復帰欄も「ファイルを直すか、別の棋譜を選ぶ」）。
   * 「開き直しても結果は変わらない」と言い切ると、直せるファイルまで捨てられる。
   */
  test("ファイルを直して開き直す道を塞がない", async () => {
    await mountWith("/ws/こわれた.kif", UNLOADABLE);

    expect(notify.mock.calls[0][0]).toHaveProperty(
      "body",
      expect.stringContaining("直してから開き直す"),
    );
  });

  /**
   * 盤に残った棋譜は表示だけになる（保存先が食い違うので門番が全部止める）。
   * **解き方はツリーで選び直すこと**で、それを書かないと「諦めて他を開け」としか読めない。
   */
  test("盤に残った棋譜を編集できる状態へ戻す操作も書く", async () => {
    await mountWith("/ws/こわれた.kif", UNLOADABLE);

    expect(notify.mock.calls[0][0]).toHaveProperty(
      "body",
      expect.stringContaining("ツリーでもう一度選んで"),
    );
  });
  ...
  /**
   * **利用者が実際に踏む筋。** 前の棋譜が載っている状態から、盤に載せられない棋譜へ
