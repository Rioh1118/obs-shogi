...
export function PositionSyncProvider({
  ...
  const getCurrentSfen = useCallback((): string | null => {
    ...
    try {
      if (!gameDerived.player?.shogi) return null;

      const sfen = gameDerived.player.shogi.toSFENString(
        gameDerived.player.tesuu || 1,
      );
      return sfen;
    } catch (error) {
  ...
  useEffect(() => {
    ...
    syncPosition().catch(() => {});
  }, [gameState.cursor, gameDerived.player, syncPosition]);
