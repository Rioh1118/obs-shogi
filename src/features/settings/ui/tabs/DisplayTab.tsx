...
/** 「起動時に開くタブ」の左の腕。**固定のタブの綴りとは別の語彙**なので、綴りを分ける */
...
export default function DisplayTab() {
  ...
  const startup = config?.dock_startup_tab ?? STARTUP_LAST;
  ...
  return (
    <div className="displayTab">
      ...
      {error && (
        <SSection tone="danger" title="保存できませんでした" description={error}>
          <div role="alert">もう一度お試しください。</div>
        </SSection>
      )}
      ...
      <SSection title="ドックのタブ" description="盤の下のドックに出す面と、その並び順">
        <ul className="displayTab__tabs">
          {tabs.map((key, at) => {
            const meta = DOCK_VIEWS.find((view) => view.key === key);
            return (
              <li key={key} className="displayTab__row">
                <label className="displayTab__check">
                  <input
                    ...
                    disabled={meta?.removable === false}
                    onChange={() => save({ dock_tabs: toggleDockTab(tabs, key) })}
                  />
                  <span>{meta?.label ?? key}</span>
                  {meta?.removable === false && (
                    <span className="displayTab__note">外せません</span>
                  )}
                </label>
                ...
                <span className="displayTab__order">
                  <IconButton
                    ...
                    variant="obs-ghost"
                    title="上へ"
                    ariaLabel={`${meta?.label ?? key}を上へ`}
                    disabled={at === 0}
                    handleClick={() => save({ dock_tabs: moveDockTab(tabs, key, -1) })}
                  >
                    <ChevronUp size={16} />
                  </IconButton>
                  <IconButton
            ...
            <li key={meta.key} className="displayTab__row displayTab__row--hidden">
              <label className="displayTab__check">
                <input
                  type="checkbox"
                  checked={false}
                  onChange={() => save({ dock_tabs: toggleDockTab(tabs, meta.key) })}
                />
      ...
      <SSection title="起動時に開くタブ">
        <SRadioGroup
          name="dock-startup"
          value={startup === STARTUP_LAST ? STARTUP_LAST : "fixed"}
          ...
          <SField label="開くタブ">
            <SSelect
              ...
              options={tabs.map((key) => ({
                value: key,
                label: DOCK_VIEWS.find((view) => view.key === key)?.label ?? key,
              }))}
            />
          </SField>
        )}
