...
type VisibleRequestBase = {
   ...
   *
   * **「あとから引っ込めたい」だけなら `dismissKey`。** こちらを取っ手に使うと、
   * 畳まれようのない通知（条件と結び付いていて、条件が消えれば引っ込む類）にも
   * 件数が出る——`Notification.count` が避けようとしている「必ず 1 件」がそれ
   */
  dedupeKey?: string;
  /**
   * 出した側が引っ込めるための取っ手。**畳まない（件数を数えない）。**
   *
   * 条件と結び付いた通知はこれを持つ。同じ取っ手の通知は**1枚に保たれる**
   * （2枚目は1枚目を置き換える）——積み上がると、引っ込めたときに
   * 1枚しか消えない形になる。
   *
   * `dedupeKey` と両方は書かない。両方あるときは畳みが勝つ（`notificationReducer`）
   */
  dismissKey?: string;
};
...
export type NotificationActions = {
   ...
   *
   * 引くのは `dismissKey` と `dedupeKey` の**どちらでも**。畳んだ通知を
   * 引っ込めたい呼び手（同じ鍵しか持っていない）を締め出さないため
   */
