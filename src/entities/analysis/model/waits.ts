/**
 * 解析の待ちの寸法。**テストから縮められる**（`shortenWaits`。**呼んでよいのはテストだけ**で、
 * `src/__tests__/testWaits.test.ts` が綴りで止める）。
 *
 * ここに在るのは全部**実時計**で測る値。テストが現物の `positionSyncTimeoutMs` を跨ごうとすると、
 * 1本あたり数秒を実時間で進めることになり、並走する機械では**コードを1行も触っていない
 * コミットが `verify` でランダムに落ちる**。落ちた側は自分の変更が壊したと読む。
 *
 * **いまは縮めるほうを選んでいる。** `act` と本物の `Promise` が噛み合う経路なので、
 * 時間を止めると待ちそのものが進まない。**偽タイマーへ寄せられないわけではない**
 * ——寄せるなら、`Date.now()` を読む打ち切りのために偽タイマー側で実時計も進める設定が要る。
 *
 ...
 *
 * **これ以上大きくしない。** 上限は実時計で測るので、縮めるほど「待ちが抜けたか」を
 * 見るテストの余白が消える——比を上げるほど上限が縮み、上限を跨ぐテストが
 * 並走する機械で落ちる。4 は、いちばん厳しいテストの経過（実測 55ms 前後）に対して
 * 余白を4倍残す値。
 */
const TEST_SCALE = 4;

/**
 * 寸法を `TEST_SCALE` 分の1に縮め、**元へ戻す関数を返す**。テストからだけ呼ぶ。
 *
