...
export function AnalysisProvider({ children, positionSync }: Props) {
  ...
  /**
   * 握れなかった席の**反映待ち**を捨てる。**掃除であって、門ではない。**
   *
   * 捨てた席の結果を画面に出さない保証は `commitLatest` の席の門が持つ
   * （`useResultFlush`）——枠を落とすだけだと、落とす前にタイマーが起きた回を守れない。
   * ここが落とすのは、次に席を握るまで枠に残る**古い1本**。
   *
   * **落とすのは席を握っていないときだけ。** 握っている回に落とすと、開始が出し直す
   * 最初の `info`（席が欄に入る前に届いた1本）まで消える。
   *
   * **この1行を落としても、いまのテストは全部緑になる。** それでも残しているのは、
   * `shoot` の catch（`keepOrForget`）が捨てた席を欄へ書き戻す回に席の門が開く、
   * という指摘があるため——**その窓は再現できていない**。消すなら先に再現を作ること。
   *
