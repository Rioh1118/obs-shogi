...
export function AnalysisProvider({ children, positionSync }: Props) {
   ...
   *
   * **枠を落とすのがここでなければならない窓がある。** 捨てる停止が落ちると
   * `shoot` の catch（`keepOrForget`）が席を欄へ書き戻すので、**その後は席の門が開く**
   * ——手前で落としていないと、捨てた席の読み筋がそこから出る。書き戻しは
   * `await stopAnalysisCore` の向こうなので、必ずここより後に起きる。
