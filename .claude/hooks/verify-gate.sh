...
# ゲートの話を書いたコミットほど止まる。
#
# **正規表現で引用を対にしない。** 正規表現は「いま引用の中か」を持てないので、
# 別種の引用の中にある引用符を開始と読む。`-m 'a "b' > X -m 'c" d'` では
# `"b' > X -m 'c"` が二重引用の中身と読まれ、**間のリダイレクトごと消える** ——
# そのあと手前も後ろも内側も見ている検査のどれにも当たらない。
# 錨づけ（トークンの先頭に限る）でも閉じない。空白の直後に来る引用符は、
# それが別種の引用の内側にあっても錨に当たる。
#
# だから状態を持って1文字ずつ歩く。
#
# 潰す条件:
#   - 単一引用符の中では**何も走らない**ので、常に潰してよい
#   - 二重引用符の中は変数展開もコマンド置換も走るので、`$` と backtick を
#     含むものは潰さない（下流に落とさせる）
#   - `all` が空なら、中に空白を含むものだけ潰す。`'git' commit` のように
#     語ひとつを引用しただけの綴りは呼び出しの一部なので残す
#   - 閉じていない引用はそのまま残す。下流の検査が拒否する
GATE_STRIP_AWK='
{
  out = ""; n = length($0); i = 1
  while (i <= n) {
    c = substr($0, i, 1)
    if (c != "\047" && c != "\042") { out = out c; i++; continue }
    q = c; body = ""; j = i + 1; closed = 0
    while (j <= n) {
      d = substr($0, j, 1)
      if (q == "\042" && d == "\134" && j < n) {
        body = body d substr($0, j + 1, 1); j += 2; continue
      }
      if (d == q) { closed = 1; break }
      body = body d; j++
    }
    if (!closed) { out = out substr($0, i); i = n + 1; break }
    runs = (q == "\042" && (index(body, "$") || index(body, "\140")))
    if (runs) out = out q body q
    else if (all != "" || body ~ /[ \t]/) out = out q q
...
gate_strip_quotes() {
  gate_flatten "$1" | awk -v all="" "$GATE_STRIP_AWK"
}
...
# **リダイレクトの記号は飲まない。** 飲むと `git -c a.b=c>path commit` の
# `>path` が呼び出しの内側に入り、手前も後ろも見ている検査の**どれにも当たらない**。
...
gate_target_dir() {
  local command=$1 base=${2:-$PWD} call flat prefix suffix
  ...
  # **呼び出しの後ろも、手前と同じ厳しさで見る。**
  #
  # シェルは `git commit` を起動する**前に**、置換もリダイレクトも済ませる。
  # だから後ろに置いても `git rm X && git commit` と結果は同じで、
  # 走査した時点のツリーはまだ変わっていない。ツリーが他に何も持っていなければ
  # 種類が空になり、**検証も deny もされないまま**コミットされる。
  #
  # **綴りを並べて塞がない。** `` ` `` `$(` `<(` `>(` を列挙した版は、
  # `>` のリダイレクトと zsh の `=( )` を素通しした。拒否リストは必ず
  # 次の綴りに置いていかれる（この関数の頭に書いてあるのと同じ理由）。
  # 手前と同じく**許す文字だけ**を見る。
  #
  # 引用の中では何も走らないので先に潰す。`gate_strip_quotes` は空白を含む
  # 引用しか潰さないので、ここでは空白の無い引用（`-m 'fix(#375)'`）も潰す ——
  # 潰さないと括弧1つで止まる。**バッククォートと `$` を含む引用は残す。**
  # 二重引用符の中では置換が走るので、そこは落とす側でなければならない。
  #
  # 2回の呼び出しに分ければ従来どおり打てる（メッセージはファイルへ書いて
  # `--file=` で渡す）。
  # **トークン境界に錨づける。** 錨が無いと、左から順に対にするので
  # 二重引用符の中のアポストロフィ2つが「引用」として対になり、
  # その間にある本物のリダイレクトごと消える
  # （`-m "don't" > X "won't"` が素通しした）。
  suffix=$(printf '%s\n' "${flat#*"$call"}" | awk -v all=1 "$GATE_STRIP_AWK")
  printf '%s\n' "$suffix" | grep -Eq '^[^;&|()<>`$]*$' || return 0

  # 呼び出しの**内側**（`git -c user.name=$(…) commit`）は上の切り出しに
  # 入らないので、畳んだ全体でも見る。
  case "$flat" in
    *'`'*|*'$('*|*'<('*|*'>('*|*'=('*) return 0 ;;
  esac
...
読むだけの動詞でも追跡ファイルを潰せる。
**リダイレクト（\`>\` / \`>>\` / \`2>\`）を同じコマンドに書かないこと** ——
コミットより先にファイルが切り詰められるので、検証したツリーと
コミットされるツリーが別物になる。出力が要るなら別の呼び出しにすること。
**置換（バッククォート / \`\$( )\` / \`<( )\` / \`>( )\` / \`=( )\`）も同じ** ——
メッセージに \`\$\` やバッククォートを入れたいなら、
ファイルへ書いて \`--file=\` で渡すこと。
