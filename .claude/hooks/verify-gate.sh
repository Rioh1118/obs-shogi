...
#
# **根拠は「コミットを作らないから」ではない。** そう言えるのは
# `--abort` / `--quit` / `--edit-todo` の3つだけで、`--skip` は中断ではなく続行、
# 残りの patch を当ててコミットを作る。
#
# 根拠は、4つとも**操作が途中のツリーでしか打てない**こと。競合を抱えていれば
# 検証は必ず落ちるので、要求すると**畳む手段そのものが取り上げられて
# 行き止まりになる。** 案内文も「再度コミットすること」になり、コミットしようと
# していない利用者に従える操作が1つも無い。
#
# `--continue` は入れない。あれは競合を解いた後の続行で、検証が通る状態で打てる
# （＝上の根拠が当たらない）。
#
