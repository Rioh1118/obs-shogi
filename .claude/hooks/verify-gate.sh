...
#
#   git -C <ワークツリーの絶対パス> commit ...
#   cd <ワークツリーの絶対パス> && git commit ...
#
# `git` の手前に何が付いていても（`timeout` など）、絶対パスで綴っても構わないが、
# **`git` と `commit` は引用符の外に、別々の語として置くこと**。それ以外の形で
# `git` と `commit` が語として現れたら、コミットかどうかに関わらず断る。
#
# **開いた文法を追いかけない。** `git -C` を拾えば前置語が漏れ、前置語を拾えば
# 絶対パスが漏れ、そこを拾えば `commit` の直後が漏れる——そのたびに
# **検証を1つも走らせずに通った**。数え上げをやめて閉じた側に倒す。
