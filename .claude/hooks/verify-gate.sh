...
# そして**deny すると出口が消えることがある** —— 競合したファイルが種類に当たれば検証は
# 落ちるので、そのとき畳む手段そのものが取り上げられる。案内文も「再度コミットすること」に
# なり、コミットしようとしていない利用者に従える操作が1つも無い。
# **必ず落ちるわけではない**（`README.md` だけの競合なら、種類に当たらないので何も走らない）。
#
...
# 1つ目が当たらず、deny されても免除済みの `--abort` で降りられるので2つ目も当たらない。
#
