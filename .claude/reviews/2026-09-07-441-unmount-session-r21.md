...
| 所見                       | 結果                                                                                    | コミット                            |
| -------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------- |
| 1 / 4 / 6                  | 直した。`engineChanged()` で reject を同じ窓へ、断りに要求の門、タイマーの欄を発火で空ける | `3401ef4b`（テスト3本、変異で確認） |
| 7                          | 直した。`bookIfRestarting` に寄せた                                                      | `b419089d`                          |
| 2                          | 直した。**`src/**` の TS コメントを見るラチェットを新設**し、掛けた時点の違反4件も直した | `17c89750`（変異で確認）            |
| 3                          | 直した。`asyncResultUse` の対象を広げ、捨てている1件に理由を書き、契約を現物に合わせた   | `725eacc1`（変異で確認）            |
| 5 / 14                     | 直した。本番から `@/__tests__/` を禁じ、`exportsTested` の起点を `walk.ts` へ            | `1b81a575`（変異で確認）            |
| 8                          | 直した。`awaitInFlightRestart` / `releaseHeldBeforeStart` に割った                       | `7b590d15`                          |
| 9〜13 / 15〜18             | 直した                                                                                  | `5cbdc961`                          |
| 範囲の外の4件              | issue #503 へ。#504 / #505 / #506 は**誤って立てた**ので `1db23729` で閉じ、`docs/IDEAS.md` を正とした（→ r22 の所見4） | —                                   |

**#503（フォント）は設計の選択が要る**——同梱するか、外部フォントをやめるか。
issue に両方の道を書いてある。

### 所見6 について

タイマーの欄を発火で空ける変更に**テストは無い**。react が「現時点で踏める経路を
作れなかった」と留保しており、こちらでも作れなかった。門の前提が別の effect の
cleanup に預けられている状態を解いただけで、振る舞いの変化は測れていない。
