...
| 4 / 12 / 14 / 15 / 17 / 18 / 19 | 直した                                                                   | `e6363b69`                          |
| 11                   | **前半だけ直した**（冒頭の名乗り）。`EXEMPT` の doc ブロック2連は残っていた → r24 で再掲 | `e6363b69`                          |
| 16                   | **部分的にしか直っていない。** 反映待ちを `useResultFlush` に閉じた（963 → 905 行、`useRef` 18 → 16、`useEffect` は 10 のまま）。自動再開の組8本はそのまま → r24 で再掲 | `86008fe6`                          |
...
- **数値は書いた直後に取り直すこと。** `ref 18 → 5` は `grep -c "useRef("` の結果で、
  **`useRef<T>(` を1つも数えていなかった**（正しくは `grep -c "= useRef"` で 18 → 16）。
- **所見が複数の文からなるときは、文ごとに結果を書くこと。** 所見11 は2文あり、
  直したのは1文目だけだった。
