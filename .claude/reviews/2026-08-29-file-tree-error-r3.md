...
**BLOCK 4件と HIGH 16件のうち、#169 の範囲にある16件を対応。**
残り4件（H-12 / H-14 / H-15 / H-16）は `main` にもある既存の欠陥なので issue へ送った。
**MEDIUM 21件のうち16件を対応、5件を issue へ。**

| #    | 結果                                                                                                                                |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------- |
| B-1  | 対応。開いているカードのスタックを持ち、閉じ込めは最上位の1枚だけが働く。2枚重ねるテスト2件を追加                                   |
| B-2  | 対応。`mv.rs` の4コマンドに `AppHandle` を足し、src と dest の両方に `validate_under_root` を通す。呼び忘れを見る Rust テストも追加 |
| B-3  | 対応。`.gitignore` に足して作業ツリーからも消した                                                                                   |
| B-4  | 対応。`cancelRef` の行と `onCancel()` の分岐にそれぞれの理由を置いた                                                                |
| H-1  | 対応。B-1 と同じスタックで Escape も最上位に絞る                                                                                    |
| H-2  | 対応。当たり判定を切り、落ちた名前は送り直さず、Escape を箱ごと拾う位置へ上げた。blur の経路のテスト2件を追加                       |
| H-3  | 対応。測れた対の下限と、測れなかった宣言の上限をラチェット。`opacity` も見る。載る面を渡す入口も足した                              |
| H-4  | 対応。読み直しの失敗は `loadFileTree` が積む1箇所に任せ、操作の戻り値には載せない                                                   |
| H-5  | 対応。解決の成否を見て、発端がモーダルなら閉じる（`AppModalLayer`）。テスト3件を追加                                                |
| H-6  | 対応。`isLoading` を足して二度押しを止めた。テスト1件を追加                                                                         |
| H-7  | 対応。3つとも結果を読む形にし、**`AsyncResult` の戻り値を捨てる呼び出しを検査で止める**                                             |
| H-8  | 対応。※1 を「再読み込みは段／逃げ道はツリーの有無」の2軸に。ADR-0005 決定5 も揃えた                                                 |
| H-9  | 対応。**ラウンド2 の反論が誤りだった。** ADR-0006 を書いて ADR-0001 を supersede し、残る3箇所を落とした                            |
| H-10 | 対応。§0 の表に3行足し、数を数え直した                                                                                              |
| H-11 | 対応。※2 を5経路の表に。`(S4, E9)` も埋めた                                                                                         |
| H-12 | **issue へ** → #214（`node.id` が UUID）                                                                                            |
| H-13 | 対応。`Spinner` への差し替えをやめ、`Button` の `isLoading` に寄せた                                                                |
| H-14 | **issue へ** → #215 の1（symlink ループ）                                                                                           |
| H-15 | **issue へ** → #215 の2（APFS の大文字小文字）                                                                                      |
| H-16 | **issue へ** → #215 の3（保存経路が `FsError` の外）                                                                                |
| M-1  | 対応。5箇所を書き直し、**経緯の混入を検査で止める**                                                                                 |
| M-2  | 対応。`submitError` に統一                                                                                                          |
| M-3  | 対応。`failWithNotice` / `failToCaller` / `failToNameInput` に改名                                                                  |
| M-4  | 対応。`commitName` が絞り込みを持ち、TSDoc と reducer に順序依存を書いた                                                            |
| M-5  | 対応。5経路とも `commitName` を通す                                                                                                 |
| M-6  | 対応。`// 6:` を落とした                                                                                                            |
| M-7  | 対応。ADR-0005 決定3 を指すように直した                                                                                             |
| M-8  | 対応。`Modal` の `size="full"` / `padding="sm"` と `Form` の `theme` 軸を落とし、`--dark` を基底に畳んだ                            |
| M-9  | 対応。条件を `dirOptions.length === 0` に揃えた                                                                                     |
| M-10 | 対応。`default` を落として網羅にした                                                                                                |
| M-11 | 対応。握りつぶされる `pushError` を落とし、残る2つに順序の理由を書いた                                                              |
| M-12 | **issue へ** → #216 の3（barrel の適用範囲）                                                                                        |
| M-13 | **issue へ** → #216 の2（パスヘルパ4本）                                                                                            |
| M-14 | 対応。`padding="none"` を渡し、中身が自分で持つ形に揃えた                                                                           |
| M-15 | 対応。`opacity` をやめて面の色で分け、`__help span` の文字を濃くした                                                                |
| M-16 | 対応。帯の面を落とし、選択中を主ボタンと同じ面に（カードに対して 3.33:1）                                                           |
| M-17 | 対応。`$color-danger-text` へ寄せ、宙に浮いたコメントも落とした                                                                     |
| M-18 | 対応。`$surface-warning` / `$surface-danger` を足し、輪郭も `FsErrorView` と同じ形に                                                |
| M-19 | 対応。`$shadow-control` / `-press` へ寄せた                                                                                         |
| M-20 | **issue へ** → #216 の1（Context と仮想化）                                                                                         |
| M-21 | **issue へ** → #215（Rust の9件）                                                                                                   |

そのほかの文書側も対応:
`failure-surfacing.md` の §1 の実測値 / §3 の F-3 の例外 / §4 の抽出条件 /
`review-round` の「件数は `CLAUDE.md`」/ `review-fix` の短ハッシュ /
Rust の `#[test]` 0個 の2文書 / append-only の境界 / #184 と #202 を TODO でコードから辿れるように。

### 自分が作った退行（先に書く）

**ラウンド2 の対応が、ラウンド3 で BLOCK 1件と HIGH 3件を生んだ。**

| ラウンド2 の対応         | ラウンド3 で出たもの                                                  |
| ------------------------ | --------------------------------------------------------------------- |
| H-7 フォーカスの閉じ込め | **B-1** 2枚重なると奪い合ってアプリが固まる（一番よく通る経路で踏む） |
| H-5 入力欄の直下に出す   | **H-2** 閉じられない箱が行の上に貼り付き、下の行を押せなくする        |
| H-11 ※2 を書き直した     | **H-11(r3)** 「3つある」が偽（実際は5経路）                           |
| H-12 台帳へ書き直した    | **H-10(r3)** §0 だけが古いまま残った                                  |

いずれも**新しく足した装置に、それ自身のテストが足りていなかった**。
`Modal.test.tsx` は1枚しか mount せず、`InlineNameEditor.test.tsx` は Enter しか叩いていなかった。
このラウンドではどちらも経路を足してある。

### 立てた issue

| #    | 内容                                      |
| ---- | ----------------------------------------- |
| #214 | `node.id` が取得のたびに作り直される UUID |
| #215 | `src-tauri/file_system` の既存の欠陥9件   |
| #216 | フロント側の構造的な宿題3件               |

### 足した装置

- `src/__tests__/asyncResultUse.test.ts` — `AsyncResult` の戻り値を捨てる呼び出しを止める
- `src/__tests__/commentHistory.test.ts` — コメントへの経緯の混入を止める
- `src-tauri` の `every_path_taking_command_checks_the_root` — `validate_under_root` の呼び忘れを止める
- `contrastRatchet` に測れた件数のラチェット
- `Modal.test.tsx` に2枚重ね、`InlineNameEditor.test.tsx` に blur の経路

テストは **30ファイル / 249件 → 34ファイル / 267件**。Rust は 4 → 5件。
