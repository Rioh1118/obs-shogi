...
**所見15件のうち14件を直した。1件は反論して直さなかった。** issue 送りは無い。

### 第0群: 集約（BLOCK 2件がその副作用で消えた）

| コミット  | 何をしたか                                                                                    |
| --------- | --------------------------------------------------------------------------------------------- |
| `add2d4e` | **書き込みを1本の列にした。** `mpsc` で受けて順に書く                                         |
| `f4be4e7` | **`Thinking` の番人を `on_tick` の1本にした。** `SearchRequest::deadline` と第1相の締切を撤去 |

集約の効き方:

- **R6-B2**（`ponderhit` に締切が付かない）は、番人が「いつ起動したか」ではなく
  「いまどうなっているか」を見る形になったので**構造的に消えた**
- **R6-B1**（締切が `stop` を送らず、探索中のエンジンへ `gameover`）も消えた。
  `on_tick` から終局させると `Activity` は `Searching` のままなので
  `finish` の `idle_sides` に入らない
- **R6-M3 / R6-M2**（投入順が保証されない／flush が追い越される）は、
  列が1本になったので消えた
- **R6-M5 / R6-B3 の一部**（18箇所の裸の書き込み）は、上限が
  `WRITE_TIMEOUT` 1箇所に移ったので全部に掛かるようになった

**番人は11個から9個に減った。** 減ったのは `STOP_WRITE_TIMEOUT`（列の上限に統合）と
`SEARCH_GRACE` の置き場（探索タスク → `on_tick`）。数より重要なのは、
`Thinking` を見る番人が2つから1つになったこと。

### 第1群

| 所見          | コミット  | 結果                                                                |
| ------------- | --------- | ------------------------------------------------------------------- |
| R6-B3 / R6-H3 | `43c75c1` | `abort()` に上限。`close_all` が失敗を返す。`Err` を doc と F-24 へ |
| R6-M1         | `de22e30` | `StopEffect::{Written, CancelledQueued}`。解析側はストリームを畳む  |
| R6-H1         | `a754c57` | 初手ぶんの加算を先に積む                                            |

### 第2群・第3群

`375737d` に R6-B1 の再発防止（`_ =>` を網羅の `match` に）と、
R6-B4 / R6-M4 / R6-M6 / R6-M7 / R6-M8 をまとめた。

### 直さなかったもの（反論）

**`HISTORY_WORDS` に `"ていなかった"` を足す案は採らなかった。**

r6 の comment レビュアーは「現物での該当は1件のみで、誤検出は出ない」と
書いていたが、**実際に足して測ると該当4件のうち3件が誤検出**だった。

- `search.rs` の「まだ書かれていなかった `go` を落とした」
- `protocol.rs` の同文
- `KifuCommentNote.test.tsx` の「閉じる手段が1つも無かった」

どれも「値がその時点でどうか」を書いており、変更の経緯ではない。
4件中3件を誤って赤くする語は、足す判断そのものを鈍らせる。
理由を `commentHistory.test.ts` の doc に残した。

### 集約の判断について

`/implement` の「ラウンドが5を超えたら、直す対象が正しいかを疑う」に従った。

数えた結果が判断を決めた。**番人11個・`send_command` の呼び出し18箇所で
上限が1箇所。** r4 と r5 は「隙間を見つけて番人を1つ足す」を繰り返していて、
足すたびに隙間が1つずれて残っていた。r6 で見つかった隙間
（`ponderhit` 経由の `Running`）も、同じやり方で塞げば7個目の番人が増えるだけだった。

**置き場を変えたら、隙間の概念そのものが消えた。** `on_tick` は
`TurnClock` を毎回読み直すので、「起動時にどうだったか」を持たない。

### 検証

`npm run verify`（645 tests）と `npm run verify:rust`（73 lib + 27）が
