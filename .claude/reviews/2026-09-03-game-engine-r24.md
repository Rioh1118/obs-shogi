...
**16件すべて着手。所見の直し方に倒さなかったものは無い。**

| 所見             | 直し方                                                                                            | コミット  |
| ---------------- | ------------------------------------------------------------------------------------------------- | --------- |
| R24-H2 / M1 / M2 | 記号の実在を注とイベントで同じに扱う／`✓` の行に名乗りを要求する／`test_column` を `## 表` に絞る | `024b6b1` |
| R24-H3           | `forbids` を**本文の綴り**で見る。`use ::tauri::` も塞ぐ                                          | `3c27cc0` |
| R24-H4           | `verify-gate.sh` の `needs_rust` に `docs/state-transitions/*` を足す                             | `dd7ef1b` |
| R24-H1           | `use_statements` の doc を戻し、拾えない範囲（素の `fn`）を書く                                   | `b743528` |
| R24-H5 / M5      | `left` が尽きていたら待たずに断る。`max(timeout, WRITE_TIMEOUT)` の断言を落とす                   | `b404f0b` |
| R24-M7           | `GameId` の `Display` で長さと制御文字を抑える                                                    | `5624706` |
| R24-M6 / M10     | 絞りの鍵を `(op, 対局)` に。上限を置き、溢れたら枠ごと捨てる。間隔の根拠も書く                    | `0c3ef17` |
| R24-M8           | 式に列の先客ぶんを入れ、`CLOSE_ABORT_TIMEOUT` を 6s → 8s                                          | `13fef98` |
| R24-M3           | 過去形の5箇所を現在の性質だけに書き換える                                                         | `89442ee` |
| R24-M4           | テスト名を、実際に表明していることに合わせる                                                      | `71359fc` |
| R24-M11          | 断る文言を `Rejection` に集める。`SIDES` の写しも畳む                                             | `8e656fb` |
| R24-M9           | `closeGame` の呼び直しに間隔と諦める条件を書く                                                    | `a9f2d2e` |

### 弱いほうに揃えなかった判断

**R24-M8。** 式に項を足すと `6 > 6` で落ちるので、選択肢は「式を下限だと
但し書きする」か「値を上げる」の2つだった。**値を上げた**（ADR-0008 決定4）。
`CLOSE_TIMEOUT < CLOSE_ABORT_TIMEOUT + CLOSE_IDLE_TIMEOUT` は保たれ、
アプリの終了は `CLOSE_TIMEOUT` で切り上げるので伸びない。

### 機械化しないと決めたもの

**R24-M3。** `commentHistory` の `HISTORY_WORDS` に「だった」「ていた」を
足すと、現在の説明を巻き込む（`src/__tests__/commentHistory.test.ts` に
理由が書いてある）。**人の側に残す。**
...
- 表の `**E16**` を `**E19**` に振り直す／`E17` の名乗りを消す／
  表の本体から `E15` の行を落とす（節で絞らない版では緑）
- `game` に `tauri::AppHandle` を**型の位置に完全修飾で**置く（`use` だけ見る版では緑）／
  `use ::tauri::AppHandle;`
- `verify-gate.sh` から追加した1行を落とす
- `GameId` の切り詰めをやめる／制御文字をそのまま通す
- `CLOSE_ABORT_TIMEOUT` を 6 秒へ戻す

### 未検証で入れたもの

**R24-H5。** `ensure_ready` は `UsiProtocol` を要求するので実プロセス無しに
呼ぶ口が無い。R23-M6 と同じ理由で、この PR では継ぎ目を作らない。
