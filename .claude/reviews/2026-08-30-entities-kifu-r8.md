...
| R8-03 | MEDIUM | comment               | `cursor` 由来の throw が `writeCandidates` の**あと**に起きるので、例外が出たのに `kifu` だけ書き換わる。`@throws` も `q` と `cursor` を1行に畳んで両方とも不正確                    | 検査の前倒しと原子性のテストは入れた。**`@throws` の分割はコミットメッセージに書いたが実際には入っていなかった**（ゲートに阻まれた bash 呼び出しの中に編集があった）。ラウンド9で R9-02 として直した |
