...
- 直し方: `message` は生ログ専用と決めて provider の日本語を code へ寄せるか、
  ...
  `error.ts` のコメントに書く。
- **結果: 対応済み（ADR-0005 の決定7）。** 前者を採った。Rust の `InvalidName` を
  原因ごとに4つへ割り、棋譜用に `kifu_format_unknown` / `kifu_parse_failed` を足した。
  **`invalid_type` が「ファイルとフォルダの取り違え」と「棋譜の解析失敗」の
  2つの意味で使われていた**ことは、この対応に入って初めて分かった。
...
所見ごとの追記は、整形で行が動くと位置がずれるのでここにまとめる。

| #         | 結果                     | 備考                                                                      |
| --------- | ------------------------ | ------------------------------------------------------------------------- |
| H-1       | 対応済み                 | reducer で編集行とメニューを畳む。テスト新設                              |
| H-2       | 対応済み                 | 提案どおりには直していない（下記）                                        |
| H-3       | 対応済み                 | M-16 の案A に吸収された                                                   |
| H-4       | 対応済み                 | `validateBasename` を entities へ出し、`pushError` へ流す                 |
| H-5       | 対応済み                 | 呼び出し元で分ける（`deferFailure`）                                      |
| H-6       | 対応済み                 | `fsErrorTier`。段で色とボタンが分かれる                                   |
| H-7       | 対応済み                 | #177 のマージ後に rebase                                                  |
| M-1 / M-2 | 対応済み                 | H-3 / H-6 の実装でコメントを書き直した                                    |
| M-3       | 対応済み                 | 案A で `message` は常に畳んだ中へ                                         |
| M-4       | 対応済み                 | 入口の `asFsError` を型ガードに。`service.ts` の11箇所を通した            |
| M-5       | 対応済み                 | reducer で衝突中は積まない。ダイアログが入力欄の下に出す                  |
| M-6       | 対応済み                 | `fallback` を渡せるようにした                                             |
| M-7       | 対応済み                 | テストを3本新設。モックを状態遷移込みに直した                             |
| M-8       | 対応済み                 | 本文に出ていないことも見る                                                |
| M-9       | 対応済み                 | `CONTRIBUTING.md` に意味色の節                                            |
| M-10      | 対応済み                 | 用途の断定をやめた                                                        |
| M-11      | **issue #180**           | `IconButton` は定義の無い CSS 変数を12個以上使う。1つだけ寄せると悪化する |
| M-12      | 対応済み（**ADR-0005**） | 5系統を1つに。面を一段暗くして白文字のまま 4.79:1                         |
| M-13      | 対応済み                 | `padding="none" scroll="content"` を明示。`max-height` を置いた           |
| M-14      | 対応済み                 | 裸の色キーワードをトークンへ                                              |
| M-15      | 対応済み（**ADR-0005**） | `FsErrorView` に集約。テーマは dark に統一                                |
| M-16      | 対応済み（**ADR-0005**） | 案A。Rust の code を細分し、文言は `describeFsError` だけが持つ           |
| M-17      | 対応済み                 | `Modal` 側1箇所でフォーカスを塞いだ                                       |
| M-18      | 対応済み                 | `!hasTree` に揃えた                                                       |
| M-19      | **issue #179**           | 読めないサブフォルダが黙って消える（Rust・差分外）                        |

### 提案どおりに直さなかったもの

- **H-2** — モーダル条件を `(error || isLoading) && hasTree` にすると、
  ファイル操作のたびに走る通常の読み直しでも通知が出る。
  「読み直しの引き金になった失敗」を持つ形にした
- **M-4** — `describeFsError` に `default` を足すと網羅 switch の型チェックが効かなくなる。
  入口の `asFsError` を型ガードにするほうが根本
- **M-5** — `conflict` を閉じる案だと、別名が不正だっただけで元の操作ごとやり直しになる
- **M-12** — 共有 `Button` に寄せるのではなく、共有側も含めて作り直した。
  文字色は黒にせず、面を暗くして白文字を保った
- **M-15** — 置き場は `AppModalLayer` へ動かしていない。モーダル版だけ移すと、
  ツリーが無いときのインライン版と2つの層にまたがる

### 途中で見つけて別に直したもの

- `npm run verify` が `.claude/worktrees/` のテストを拾っていた（追跡下15に対して29ファイル）
- `tsconfig.test.json` に `vite-env.d.ts` が無く、テストから `.scss` を読むと丸ごと TS2307
- `FileConflictDialog` が `onSubmitRename` の結果を受け取りながら捨てていた

### 数字

- SCSS の直値が **118件**減った（`font-size` 251→218 / `spacing` 528→479 ほか）
- テストが **143 → 210件**（`.claude/worktrees/` を除いた実数）
- ボタン定義が **5 → 1**。削除は約1,100行
