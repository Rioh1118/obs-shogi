...
- 結果: 対応済み `f449a95d` — 説明を、変異が実際に死ぬ rerender の上へ移した
...
- 更新の知らせの境界も同じ座標なので、両方畳まれると後勝ちで片方に触れなくなる（r2-04 が
  位置を動かしただけで解けていない）。
- 結果: 対応済み `dd8e52c5` — ヘッダの下へ移した（r3-03 と同じ編集）
...
- 結果: 対応済み `dd8e52c5` — `$z-error-fallback` を通知の上に1つ足した
...
- 結果: 対応済み `1ff2246d` — 3枚とも「…してから『再表示』を押してください。」の形にした
...
- 結果: 対応済み `c81d0697` — `fallback` の引数に `hint` / `floating` を足した（r3-19 と同じ編集）
...
- 結果: 対応済み `26ccf720` — 過去形を落とし、守るべき条件（URL を持たない入力を足すときは鍵にも足す）を書いた
...
- 結果: 対応済み `bb5d87f2` — 共有の理由を「2つの帯が同じボタンを別々に描くから」に、半透明の理由を `backdrop-filter` と対の意匠に直した
...
- **結果: rebase 済み**（下記）。
- 結果: **対応済み（rebase）** — `origin/main`（`febcee04`）へ rebase し、`failure-surfacing.md` の衝突は main の「通知」の行を残して解決。ADR-0003 の実測（35 / 22）は rebase 後の木で一致を確認
...
- 結果: 対応済み `c113ef4c` — 境界を `AppModalLayer` の中へ入れ、`AppLayout` から `useFileTree` の購読を落とした
...
- 「読む入力を全部」は**足し忘れる形の規則**。`location.key` / `location.search` に寄せると、
  失敗の形そのものが消える。
- 結果: 対応済み `c113ef4c` — 鍵を `location.key` に寄せた。param を名前で数え上げる形をやめた
...
- 結果: 対応済み `c113ef4c` — `AppModalLayer.test.tsx` に実物の鍵を見るテストを2本足した（`conflict` を鍵から落とす変異で落ちることを確認）
...
- 結果: 対応済み `ff24e5ae` — `onClick` を `() => void | Promise<void>` に広げ、拒否を部品側で握るようにした
...
- 結果: 対応済み `6047c678` — `location.key` と React の `key` を書き分け、根拠を「遷移を起こす導線が残らない」に寄せた
...
- 結果: 対応済み `c80809c1` — 「入れ子の何枚目でも」に直して数を落とした
...
- 結果: 対応済み `da3214a7` — 4.19:1 / 1.56:1 に直し、内側が 2.69:1 で境として効いているのは地の側だけ、と書いた
...
- 結果: 対応済み `da3214a7` — 「子が閉じている間 `null` を返し、開いたときだけ `Modal` が portal する」に直した
...
- `__closeError` は `$surface-fatal` + `$color-fatal-text` で `InlineNotice` と同じ役割の箱を手で組み直したものだが、
  **なぜ土台を通さないのかがどこにも書かれていない。** `CONTRIBUTING.md` は
  ...
  明文で求めている。
- 結果: 対応済み `83660ae3` — `actions` → `extraActions`、`notice` → `afterAction`。土台を通さない理由も書いた
...
- 結果: 対応済み `09a07320` — 鍵の中身は呼び出し側が持つ、と書いて写しをやめた
...
- 結果: 対応済み `c81d0697` — `actions` / `notice` だけが本文側に残る線引きを doc に書いた（どちらも fallback の state に依存する）
...
- 結果: 対応済み `b6fbbec8` — `$app-header-height` を1つ置き、`.app-header` の再宣言を落とした
...
- 結果: 対応済み `5a3dcbe3` — 帯の左 padding もトークンにした
...
- 結果: 対応済み `09a07320` / `27277529` — #514 を `app-layout.md`、#515 を `boot.md`、#405 を `system-dialogs.md` に足した
...
- 結果: 対応済み — #514 の本文の引用を現在の一文に差し替えた
...
- 結果: 対応済み `b975edc2` — 2本とも `## 重複・矛盾した所見` に戻した
...
- 結果: **見送り** → issue #522。リポジトリ入口の4件。#436 の範囲の外で、どれも `main` から在る
...
| r3-25 | **issue #522**         | リポジトリ入口の4件。#436 の範囲（境界を置く）の外で、どれも `main` から在る                                                                                               |
