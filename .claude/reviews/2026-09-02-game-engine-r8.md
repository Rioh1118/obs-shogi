...
| コミット  | 直した所見                                                          |
| --------- | ------------------------------------------------------------------- |
| `ae55294` | R8 の集約2つ ＋ B1 / B2 / B3 / B4 / H4 / H6 / H7 / M1〜M6 / M8〜M12 |
| `849f773` | R8-H5 / R8-M7（出口の無い失敗に F 番号を添える）                    |

### 集約

**1. `dispatch_for` を3引数に畳んだ。**

送る／積む／断るを決めているのは `(ReadyState, draining, cmd)` の写像なのに、
テストが当たっていたのは2引数で、`draining` の判断は本文側にあった。
そのせいで `requires_ready` の3コマンドしか積まれず、
`gameover` と `ponderhit` が積み置きを追い越していた。

写像の表（24行）をテストにした。**バリアントやコマンドを足せば行が増える。**

**2. `WRITE_TIMEOUT` を `run_writer` の中へ移した。**

待っている側で包むと列の待ち時間が入るので、
「ここに達するのはエンジンが stdin を読んでいないときだけ」は偽だった。
移したことで `Err(Timeout)` が「その1件が本当に書けなかった」を意味するようになり、
`fail_writes` の根拠が初めて成立する。

### 機械が6回、私を止めた（累計）

このラウンドだけで2回。`docsIdentifiers` が `SHUTDOWN_TIMEOUT` を、
`timeout_result` が新しい `is_err()` を捕まえた。

### 残した判断

**F-26（`fail_writes` でエンジンが使えなくなる）は台帳に載せるところまで。**

根を直すには「`Closed` を立てるだけでなく台帳からも外す」が要るが、
そうするとフロントの既存の「起動していない」経路に乗る一方で、
**利用者に何を出すかは対局 UI と一緒に決めるべき設計**になる。
UI が無いいま決めると、決めたことを確かめる手段が無い。

### 収束について

件数は 20 / 17 / 16 / 15 / 16 / 20 で減っていない。ただし**中身は変わっている。**

- r4〜r6: 番人と番人の隙間（設計の穴）
- r7〜r8: 集約の副作用と、**私が書いた doc の断言**

r8 の集約は後者に効くはず。`dispatch_for` の表は
「`draining` 中に何が積まれるか」を文章ではなく写像で言う形になった。
**次のラウンドで、同じ形の所見が減るかを見る。**

### 検証

`npm run verify`（645 tests）と `npm run verify:rust`（75 lib + 31）が
