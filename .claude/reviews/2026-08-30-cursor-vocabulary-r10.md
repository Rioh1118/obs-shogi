...
| C3  | comment      | `reachedCursor` に本番の呼び出し側が無いことが3つの doc に書かれていない | **一部だけ直った**（下記） |
...
**【r11 で判明】C3 は `playerCursor.ts` にしか入っていなかった。**
`branch-index.md` への編集は、検証ゲートで止まったコミットの巻き戻しで消えていた
（CLAUDE.md の `/implement` が警告している形：「コミットが gate で止まると
lint-staged がワークツリーを復元し、その巻き戻しで直前の編集が消える」）。
このラウンドの報告書は「両方に足した」と書いていたが**誤り**。r11 で直した。
