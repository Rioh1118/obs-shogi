...
### ~~カードの**幅**にも同じズレがある~~ — 誤り。ラウンド2で取り消す

r1 でここに「`width: 96vw` は overlay の内容幅 `W - 40`px を `W < 1000px` で超えるが、
害が小さいので直さない」と書いた。**超えない。** ラウンド2の `ui-reviewer` が
Chrome headless で実測して否定した。

カードは overlay（`display: flex`、主軸は横）の flex item なので、幅は主軸サイズであり
`flex-shrink: 1`（既定）が効いて内容ボックスに収まる。実測値:

| viewport 幅 | `96vw` | 実測カード幅        |
| ----------- | ------ | ------------------- |
| 1280        | 1228.8 | 1228.8              |
| 800         | 768.0  | 760.0（= 800 − 40） |
| 600         | 576.0  | 560.0（= 600 − 40） |

**この非対称こそが、高さだけ `100%` で挟む必要があった理由。** 主軸は shrink が効くので
勝手に収まり、交差軸（高さ）は shrink が無いので溢れる。r1 の書き方だと、読んだ人が
`max-width: 100%` を足す（無意味）か、逆に高さの `100%` も不要だと誤読する。

幅を触らないという判断そのものは変えない。
