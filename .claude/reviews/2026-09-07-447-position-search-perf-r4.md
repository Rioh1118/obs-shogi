...
- 結果: 対応済み（`b04e5b4a` 結末を `SearchLaunch` の判別可能な union に。捨てる側は `cancel_search` も投げる。Promise チェーンを1つの async に畳んだ）
...
- 結果: 対応済み（`1e44b858` 状況バーに実際に出る理由の文字列を見る。門を外すと落ちることを確認）
...
- 結果: 対応済み（`cb3cf304` `discardSearch` が覚えている問い合わせまで落とす。StrictMode のテスト1本）
...
- 結果: 対応済み（`b8ccc466` 3つの理由と段の決め方を書いた）
...
- 結果: 対応済み（`63e5d90b` `never` 落ちの switch に。`DropSearchAction` を `reducer.ts` へ移した）
...
- 結果: 対応済み（`aae921a5` `EVT_*` の値 export を落とした）
...
- 結果: 対応済み（`40411f03` バッジも `Record` に。段を1つ足して2つの表が両方落ちることを確認）
...
- 結果: 対応済み（`aae921a5` lint が禁じている形を指示していたのを直した）
...
**ユーザーの判断でここで PR に出す。** r4 の12件はすべて対応済みで、`main`
（`89ba0270` 定跡の土台）へ rebase 済み・`npm run verify` は緑。

「指摘ゼロのラウンドが出るまで」（`/implement` 手順5）は満たしていない。
r4 の計画が書いた「次ラウンドの焦点」6点は**未消化のまま残っている**——

1. R4-1 の型で、正常な検索が捨てられていないか。`cancelSearch` が二重に飛ばないか
2. R4-3 で撃ち直しが増えすぎていないか
3. R4-6 の `never` 落ちが、既存の2つの合図で正しく通ること
4. R4-7 で barrel から `EVT_*` を落とした後、スライス内の購読が壊れていないか
5. 今回直した doc が、また別の場所で双子を作っていないか
6. 変異を当てていないテスト4本（`kifuCache` / `useOrderedPositionHits` /
   `appendOnly` / `PositionSearchContinuation`）が、別の理由で緑になっていないか

**続きは PR のレビューで拾う。** ラウンドごとの所見数は 26 → 21 → 11 → 12 で、
r3 / r4 はどちらも「前のラウンドの修正が作った HIGH」を1件ずつ含んでいる。
r4 でその機構（非同期の世代）を型へ寄せたので、次のラウンドで同じ形が出るかが
判断の分かれ目になる。出るなら `docs/IDEAS.md` の `useSearchSession` を実行する。
