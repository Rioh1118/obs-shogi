...
- **`structuredClone` が「正規化の部分書き換えから原本を守っている」**: comment と robustness が
  そう述べ、robustness は「外しても全件 green」を根拠にテスト追加を求めた。実測すると、
  KIF 入力では `normalizeMinimal` は**入力を書き換えずに** throw する（tsshogi が同・相対表記を
  先に埋めるため書き込みが no-op になる）。CSA は非合法手を import 段階で弾く。
  つまり現在の入口では部分書き換えは起こせない。`structuredClone` は残すが、コメントを
  「実質起きないが原本を守る責任をここに置く」に直した。テスト名も、実際に保証している
  「tsshogi の出力がそのまま返る」に改めた。
