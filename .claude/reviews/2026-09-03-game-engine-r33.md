...
| 所見             | 直し方                                                                     | コミット   |
| ---------------- | -------------------------------------------------------------------------- | ---------- |
| R33-H4           | 経緯を4箇所から落とし、`HISTORY_WORDS` に2語足す                           | `e69ec857` |
| R33-H3           | 等式を「実際に埋まる量」で留める。`LOG_FILE_BUDGET` の doc の嘘を直す      | `ad5f29a8` |
| R33-H1           | `over_line` が自分でも切る（順序に頼らない）。3つの吸い口を全部見る        | `a8380390` |
| R33-H5 / M6      | 着手の断りを2つに分ける。`Failed` の切り詰めをログの実引数へ               | `c4fb1f3d` |
| R33-H2 / M3      | `with_cause` で原因の連鎖を出す。`AlreadyListening` をバリアントで判定     | `c0a87295` |
| R33-M1 / M8      | 下側を通したい実例で押さえる。`MAX_PATH_IN_LOG_LEN` へ改名                 | `c3477ffc` |
| R33-M2 / M4 / M5 | `op` をコマンド名へ。表示名の契約を TS と台帳へ。絞りを通らない1行を落とす | `93077b45` |
| R33-M7 / M9      | 残った数字の写しと ※8                                                      | `55fb3825` |

### 順序を表明で留めず、頼らない形にした

R33-H1 の所見は「3つの吸い口を見るテストを書き、(c) が順序を固定する唯一の表明になる」を
薦めていた。**書いたうえで、順序に頼らない形にした。** `over_line` が自分でも切れば、
切る行がどこにあってもログは収まる。表明で留めると「表明を消せば戻る」が残る。

実際、3つの吸い口を見るテストだけでは**順序の変異を落とせなかった**（テストは
`runner.phase` から線を組み直すので、既に切れている値を測る）。書いた直後に測って気付いた。

### `commentHistory` に語を足したのは two-strikes を満たすから

「コメントに経緯を書く」はこの repo で既に1度ラチェットを生んでいて、今回はその網を
**すり抜ける言い換え**で同じことをした。網を広げるのは新しい機構ではない。

`無かった` も試したが、「書く必要が無かった」のような**いまの条件の説明**に6件当たるので
入れていない。
...
どちらも上限の不等式に余裕があるためで、**測る側を小さくする間違いはこの形では
落とせない**（ラウンド31から同じ限界を doc に残している）。
