...
27件（重複を除くと21件）すべてを処理した。**そのほとんどがラウンド17の自傷。**

### 走査の故障（これが緑なので他が見えなかった）

| 所見    | どう直したか                                                                                      |
| ------- | ------------------------------------------------------------------------------------------------- |
| R18-B1  | 括弧の対応取りに文字列・raw 文字列・文字・コメントの読み飛ばしを入れ、**`None` を黙って通さない** |
| R18-H5  | 署名の丸括弧を対応で取る（`Channel<()>` で切れない）。3箇所が同じ関数を使う                       |
| R18-M13 | `mod` の入れ子を数えて `super` を解決。**段に無い行き先も報告する**                               |
| R18-M17 | `;` は角括弧と丸括弧の外でだけ数える                                                              |

`tests/scanning/mod.rs` に寄せた。**別々に書くと同じ穴が3つできる**ことが、
このラウンドの所見そのもの。
...
| 所見         | どう直したか                                                                      |
| ------------ | --------------------------------------------------------------------------------- |
| R18-M3 / M14 | 全キーを1つの集合に潰すのをやめ、**型ごとに**突き合わせる。見本の足し忘れも拾う   |
| R18-H4       | doc の「245行」「誰も触っていない」は3つとも偽だったので落とした                  |
| R18-M2       | `on_tick` の配線を通るテストを足した（隣り合う2つの `bool` を入れ替えると落ちる） |
| R18-M5       | 効いていない `enforce_engine_timeout = true` を落とした                           |

### 対局が壊れる

| 所見    | どう直したか                                                                   |
| ------- | ------------------------------------------------------------------------------ |
| R18-M1  | `initial_moves` を `>=` で弾き、2つの境界の関係をテストで固定                  |
| R18-M15 | `start_sfen` の長さと `options` の件数・長さを入口で見る                       |
| R18-M6  | 各段に残りを渡して縮めさせる。**厳密な上限ではない**ことを3箇所の doc に書いた |
| R18-M16 | `kill_engine` を3値にして、落とし切れなかったことを `error` で残す             |
| R18-M4  | 「閉じている最中」を「知らない対局」と言わない                                 |

### doc

R18-H1（doc の引き剥がし2箇所。`pub async fn close` が `///` を失っていた）、
H2（※12 の `has_spoken`）、H3（※3 と F-28 の検算の一覧）、
M7〜M12 をすべて現物と突き合わせた。

### 変異で確かめたもの

- `mod tests` の後ろに `.unwrap()` を書く → `production_code_has_no_bare_unwrap` が落ちる
  （行番号も現物と一致）
- `Channel<()>` を挟んだ `file_path: String` のコマンド → `root_guard` が落ちる
- `super` の深さの取り違え → 2つの検査が `game -> events` を偽の辺として落とす
- `GameSnapshot` に `detail`（`GameResult` に同じ綴りがある）→ 型ごとの検査が落ちる
- TS から `AnalysisCandidate.pv_line` を落とす → **1欄だけ**が報告される
- 宣言の途中に空行 → 欄が消えないこと
- `initial_moves` を `>` に戻す → 1手も指せない設定が通る
- `on_tick` の2つの `bool` を入れ替える → 最後の上限が消えることが落ちる
- `start_sfen` の長さの検算を外す → 落ちる
