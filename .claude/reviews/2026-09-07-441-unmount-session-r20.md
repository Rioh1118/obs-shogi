...
| 所見                 | 結果                                                                                  | コミット                            |
| -------------------- | ------------------------------------------------------------------------------------- | ----------------------------------- |
| 1（結果の取り残し）  | 直した。席を捨てる枝の後始末を `landed` の1箇所に集めた                               | `70b981fb`（テスト1本、変異で確認） |
| 3 / 4 / 18           | 直した。世代をフックへ、`beginTake` / `landed`、`onEngineGone` の分離、引数の改名     | `70b981fb`                          |
| 2                    | 直した。往復の前後で世代を見て、消えたエンジンの席は書き戻さない                      | `5e1f4307`（テスト1本、変異で確認） |
| 1（断りと予約）/ 19  | 直した。`ENGINE_RESTARTED_MESSAGE` と、同期の追従の門を `runRestart` と揃える         | `14c6ad1a`（テスト2本、変異で確認） |
| 8                    | 直した。`readinessRef` を1本にして `??` を書けなくした                                | `86f01d43`                          |
| 15 / 17              | 直した。`keepWaitingForSync` / `swapSeatAndGo` に割り、剥がれた doc を戻した          | `5712db3c`                          |
| 7 / 9 / 10 / 11      | 直した。接頭辞の区切り・`*.ratchet.test.ts`・走査範囲の一本化。どれも変異で確認       | `5c0b9a5e`                          |
| 5 / 6 / 12 / 13 / 14 / 16 | 直した                                                                           | `07fed0fa`                          |
