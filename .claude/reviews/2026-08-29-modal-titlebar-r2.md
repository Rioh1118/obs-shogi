...
| ---------------- | ------- | --------------- | -------- | ---------- |
| 変更前 `bb29884` | 12..276 | 13..237         | 185..237 | 200..223   |
| `c2e422e` 時点   | 46..268 | **47..272**     | 220..272 | 235..258   |

パネルがカードを 4px 超え、フッタの下端がその分だけ切られる。切られる量は `66 - 0.22H`。

> **ラウンド3で一部取り消し。** r2 はここに続けて「フッタ下端と保存ボタン下端の差は
> 14px なので viewport 高 236px を切ると保存ボタンが完全に隠れる」と書いたが、
> これは外挿であって実測ではない。上の表でも保存ボタンの下端 258 はカードの
> 下端 268 の内側にあり、この時点では切られていない。
>
> r3 の `comment-reviewer` が実測し、236px は修正前にボタン下端が数 px 欠け始める
> 高さであること、ボタンが完全に消える高さでは**この修正を入れても消える**ことを
> 確認した。パネルがカードを超えてフッタが切られること自体は実在するので、
> `7d62b96` の判断（他の4つと同じくカード基準に揃える）は変えない。
> SCSS 側のコメントからは検算できない閾値を落とした（`10248bb`）。
...
> **ラウンド3で訂正。** ここで書いた「`88vh` で H < 217px」を `--size-md` / `--size-lg` の
> 直上に置いたが、その2つは `@media (max-height: 720px)` が先に効くので `88vh` が生きるのは
> H > 720 のときだけ。はみ出しの条件は H < 550 なので両立せず、**到達できない**。
> 217px が成立するのは `@media` の対象外である `--dialog.modal--size-xl`。
> `--size-sm` の 130px は正しい。`237c8cb` で導出と適用範囲を書く形に直した。
