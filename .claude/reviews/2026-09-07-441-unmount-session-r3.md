...
| 所見                       | 結果                                                                            | コミット / 送り先                   |
| -------------------------- | ------------------------------------------------------------------------------- | ----------------------------------- |
| 1 (BLOCK)                  | 直した。型 import を barrel へ。以後 `npm run verify` は終了コードを見る        | `c6448e54`                          |
| 2 / 11 / 12                | 直した。go の前にも門、同期待ちに脱出、後始末を門の後ろへ                       | `054290a4`（テスト3本、変異で確認） |
| 13 / 14                    | 直した。返せなかった席を握り直す。打ち切りの門をテストで固定                    | `da064a68`（テスト2本、変異で確認） |
| 9 / 10 / 12(doc)           | 直した。`state.sessionId` を落とし、S0 / S6 / ※1 を席の欄で書き直した           | `d77feb1f`                          |
| 3 / 4 / 5 / 6 / 7 / 8 / 16 | 直した。理由の正を ※12 に1つ置き、コードのコメントはそこを指す形に              | `d77feb1f`                          |
| 15 / 17                    | 直した。`stop_all_sessions` に呼び手の名前。Rust の「項目」を「席」に寄せた     | `4f20c321`                          |
| 21                         | 直した。`useEngineSeat` に閉じた。**テストは1行も変えずに緑**                   | `58b35e0b`                          |
| 19                         | #356 にコメントを付けた（前半は ADR-0007 で解決済み、生きているのは発火元の話） | —                                   |
| 20                         | **issue #471** へ。render 中の ref 代入をどう畳むかは設計の選択                 | —                                   |
| 22 / 23                    | 直した                                                                          | `e564e78f` / F-7 の書き直しに含む   |

**1件、計画から外れた。** 所見9 / 10 / 12 と 3〜8 / 16 を `d77feb1f` の1コミットに
まとめてしまった（`git add -A` で拾った）。1コミット1所見の約束から外れている。
`/tidy-commits` で PR 前に割り直す。
