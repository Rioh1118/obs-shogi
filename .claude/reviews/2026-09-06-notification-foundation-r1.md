...
r1 の 41件すべてを処理した。コミットは 20 本（束の中で1所見1コミットは崩していない）。
検証は手で走らせた（このワークツリーでは門番が働かない → #394 / #342）。

### 直したもの

| 所見                              | どう直したか                                                                                                    | 変異で落ちることを確認              |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| H1 / H8 / M1                      | `NotifyRequest` を見せ方の判別 union に。3件が同時に消えた                                                      | 型で4通りの誤りが弾かれることを確認 |
| H2                                | modal の `Notice` に `key={modal.id}`                                                                           | ✓（先にテストで再現）               |
| H3 / M3                           | `running` を添字から action の同一性へ。`mounted` ref を削除                                                    | ✓ 2件                               |
| H4 / M7                           | 通知をモーダルより上へ。段を `index.scss` のトークンに。`modalOverlayTitlebar` に大小と `pointer-events` を追加 | ✓ 2件                               |
| H5                                | クリックを受けるのを入れ物から通知1枚ずつへ                                                                     | 実効 CSS で確認                     |
| H6                                | 通知のモーダルを `scroll="content"` へ                                                                          | —                                   |
| H7                                | 新着を入れ物の先頭に置く                                                                                        | ✓                                   |
| H9                                | `NotifyAction.failureBody` を足し、原因を `console.error` へ                                                    | ✓ 2件                               |
| H10 / M29                         | 経緯のコメントを落とし、ポータル先のコメントをその行の話に                                                      | —                                   |
| H11                               | `Omit` に `onDismiss` を足す                                                                                    | 型で確認                            |
| M2                                | context を2つに割り、`useNotify` を新設                                                                         | ✓                                   |
| M4                                | 畳むとき `actions` を上書きしない（「書いていない」と「空で書いた」を区別）                                     | ✓ 2件                               |
| M5                                | 常設の live region を層に置く                                                                                   | ✓ 2件                               |
| M6                                | 走行中も押せる状態にし、二重起動は `invoke` で弾く。`Button` に `busy` を追加                                   | ✓ 2件                               |
| M8 / M9                           | 帯を `flex: none` で先に確定。帯の見た目を `notice--banner` へ                                                  | —                                   |
| M11 / M22                         | 帯が器を覆うことを SCSS と仕様に。通知の modal を在庫に数える                                                   | —                                   |
| M12 / M20 / M21                   | CONTRIBUTING の表を基準ごとに割り、土台への入口を先頭に。`FsErrorView` を #180 の一覧へ                         | —                                   |
| M14                               | `fsErrorTier` の戻り値を `VisibleTier` から取る                                                                 | 段を改名すると3箇所落ちることを確認 |
| M17 / M18 / M19                   | 台帳の「出口」の意味を1つに戻し、予測を落とし、測定日を添える                                                   | —                                   |
| M23                               | コメントの比を実測に直す（4件とも基準か値が違っていた）                                                         | 自分でも再計算                      |
| M24 / M25 / M26 / M27 / M28 / M30 | cleanup の誤説明、作業単位を指す語、公開面の TSDoc、語彙の規則、数値の由来、写経の集約                          | —                                   |

### 提案どおりに直さなかったもの（理由を残す）

| 所見 | どうしたか                                                                                                                                                                                                                                       |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| M13  | **型を寄せなかった。** `NotifyAction` にすると `run` が `Promise` を返せるようになるが、`FileTreeErrorNotice` には**その失敗を出す場所が無い**。沈黙する経路を1つ消して別の1つを作ることになる。寄せる条件を `TODO(#277)` として書いた           |
| M10  | **画面ごとの切り替えをしなかった。** 帯の有無を層へ伝える手段が `:has()` かグローバルな class の付け外ししか無く、リポジトリに `:has()` の先例が1つも無い（3つの webview を跨ぐ判断になる）。2.6rem の隙間より高くつくので、制約を SCSS に書いた |
| M15  | **`UpdaterScreen` を器の中へ入れなかった** → #432。更新画面を通知へ載せ替える判断と同じ場所を触るので、先に入れると #432 でもう一度触る。外に残っていることを `BootstrapProviders` の doc に名指しした                                           |
| M16  | doc の1行だけ。`FsErrorView` を `Notice` へ寄せるのは ADR-0004 の「移行を一度にやらない」の範囲                                                                                                                                                  |

### 計画が予告した「壊しうるもの」の実測

- **union → 既存テストが型エラーになる**: 起きた。3ファイルのヘルパを書き換えた（`presentation` を差し替え可能にすると spread の結果が枝に決まらないため、口を分けた）
- **live region → 本文が DOM に2回出る**: 起きた。`NotificationLayer.test.tsx` の2本と `BootstrapProviders.test.tsx` の2本が `getByText` で多重一致になり、入れ物を指す形へ直した
- **`scroll="content"` → 余白が二重になりうる**: 起きなかった（`.modal__body` は padding を持たない）
- **層を上げる → モーダルのクリックをトーストが食う**: 狭いウィンドウでのみ。`pointer-events` を1枚ずつにしたので面では食わない。r2 の焦点
- **`running` を identity に → action を共有すると両方 busy**: 呼び出し側が毎回作る前提。r2 の焦点

### このラウンドで自分が作った退行

- `Array.prototype.at` を使い、`tsc` の lib（ES2020）で落ちた。**テストだけ走らせて `verify` を通していなかった**ため気づくのが遅れた
- `commentHistory` が自分のコメントの「〜ようになったら」を捕まえた（機械が効いた）
- `python` の文字列置換が **CONTRIBUTING.md で2回黙って空振りした**（前のコミットの整形で表の桁が変わっていた）。以降は `Edit` を使い、失敗が見えるようにした

### #272 のフレークの再測

r1 の修正中に **4回**踏んだ（`verify` の終了コードが 1 になるが、テストは全件通っている）。
テストファイルを5本足したので頻度が上がっている。#272 にコメント済み。
