...
**所見16件をすべて直した。** issue へ送ったものは無い。

### 第0群: 根を直した

| 所見          | コミット  | 結果                                                                                                                                           |
| ------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| R5-B2 / R5-H3 | `0f61ba0` | エンジンへの書き込み4箇所を `spawn_blocking` へ出した。**これで r4 の上限が初めて上限になった。** r1 の M-4（5ラウンド持ち越し）も同時に閉じた |

**第0群を先にやったのは正しかった。** r4 は「上に積む」修正を4件入れて4件とも
片側しか塞げなかった。根（同期ブロッキング write）を直した後は、
残りの修正が素直に入った。
...
| 所見  | コミット  | 結果                                                                        |
| ----- | --------- | --------------------------------------------------------------------------- |
| R5-H1 | `071f61a` | `outcome_of_stop_write` に切り出して3分岐に                                 |
| R5-B3 | `e7e2cd9` | `pending` を取った後に `dispatch_for` を引き直す。`kill_engine` に `Closed` |
| R5-B1 | `8308a22` | `stop` が積み置きの `go` を取り消す                                         |
| R5-B4 | `dd69f28` | 閉じられなかったセッションを台帳へ戻し、`Err` を返す                        |
| R5-H2 | `c494e86` | 第1相に締切（持ち時間＋`SEARCH_GRACE`）。**H-3 が5ラウンド目で閉じた**      |

### 第2群: 機械化

| 所見  | コミット  | 効いた証拠                                                                              |
| ----- | --------- | --------------------------------------------------------------------------------------- |
| R5-H1 | `13a4a57` | `timeout_result` が**作った直後に `registry.rs` の2件を捕まえた**（どちらも正当な例外） |
| R5-C7 | `4cda2be` | `commentHistory` の自己免除を外したら、**自分の doc の3箇所が落ちた**                   |
| R5-C8 | `4cda2be` | `docsIdentifiers` を語境界に。部分一致だと `FOO` → `STOP_FOO` の改名を見逃す            |

### 第3群: doc

`a5e5b13` に R5-C1〜C6 / C9 をまとめた。`13a4a57` に R5-M1。

### この修正で確かめたこと

**「上限を置いた」と書いたら、それが本当に上限かを確かめる。**
r4 の4件はどれもコードとしては正しく見え、テストも緑で、
`tokio::time::timeout` を正しい引数で呼んでいた。**効かないのは
内側が同期ブロッキングだったから**で、それは呼び出し側からは見えない。

この形は機械で防げない（r5 の lint 案にも「機械で防げないもの」として挙がっている）。
代わりに残せるのは、**なぜ効くのかを書くこと**。
`registry.rs` の `WRITE_TIMEOUT` と表の ※4 に「書き込みが `spawn_blocking` の
中にあるから効く」を足したのはそのため。async の中の同期 write へ戻す変更が、
「上限があるから安全」を根拠に通らないようにする。

### 検証

`npm run verify`（645 tests）と `npm run verify:rust`（72 lib + 2 + 2 + 9 + 10 + 4）が
