...
31件（重複を除くと24件）すべてを処理した。

### ラウンド19の退行

| 所見       | どう直したか                                                             |
| ---------- | ------------------------------------------------------------------------ |
| R20-H10    | 取り分を先に決めてから引く。`usiok` に何も残らないなら**起こす前に**断る |
| R20-M9/M11 | 上限の判定を接頭辞・末尾・偶奇の**後ろ**へ移す                           |

**既存テストが誤った順序を仕様として固定していた**ので、写しを `MAX_PLIES` 手に
してから1手足す形に直し、「接頭辞の合わない長い列は `Err`」を対で足した。

### root ガードと走査

| 所見   | どう直したか                                                                        |
| ------ | ----------------------------------------------------------------------------------- |
| R20-B1 | 文字列も潰した写しを `commands` が別に返し、**関門の判定だけそちらに向ける**        |
| R20-H6 | `strip_line_comments` を捨てて `blank_out_comments` に載せる                        |
| R20-H7 | ブロックコメントの入れ子を数える                                                    |
| R20-H8 | 判定をコメント除去後に、`read_dir` を再帰に、**一覧をやめて述語に**                 |
| R20-H9 | `#[derive(..)]` を `]` まで連結してから見る                                         |
| R20-M8 | `br"..."` を raw として読む（先頭が `r` かでなく、接頭辞に `r` が含まれるかで見る） |

**述語に変えたら5つ目の手書き走査（`comment_identifiers`）が出てきた。**
一覧のままなら見つからなかった。

### 踏めるのに踏んでいなかった枝

R20-M13 の11件のうち重い3つ（`on_search_info` の `is_to_move` /
`accept_continue` の `turn_clock = Settling` / `RULING_TIMEOUT`）と、
R20-M12（偶奇の番人に届いていなかったテスト）を埋めた。

**表の「実プロセスを要するから固定できない」という前置きが、
実プロセスを要さない11件を「仕方ない側」に寄せて見せていた。**

### doc

R20-H1〜H5 / M1〜M7 / M14 をすべて現物と突き合わせた。
散文の秒数を定数名で指す（`RULING_TIMEOUT` が8箇所、他2箇所）。

### 変異で確かめたもの

- 関門を消してログに綴りだけ書く → `every_path_taking_command_checks_the_root` が落ちる
- doc コメントの中の `#[cfg(test)]` の後ろの `.unwrap()` → 落ちる
- `Channel<()>` / `fn f<F: Fn() -> String>(..)` を挟んだ生パス → 落ちる
- コメント行の `mod tests {` → 段の検査が生きたまま落ちる
- 折られた `#[derive]` の tagged enum → ADR-0007 の検査が落ちる
- 門番の抜け道3つ（コメントで免除 / サブディレクトリ / `find("//")`）→ すべて落ちる
- `on_search_info` の `is_to_move` / `turn_clock = Settling` / `RULING_TIMEOUT` の
  3つの変異 → それぞれ対応するテストが落ちる
- 偶奇の `if` を削る → `continue_is_refused_when_the_move_count...` が落ちる
