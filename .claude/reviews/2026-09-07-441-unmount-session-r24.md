...
| 所見              | 結果                                                                                   | コミット                            |
| ----------------- | ---------------------------------------------------------------------------------------- | ----------------------------------- |
| 1 / 4 / 9         | 直した。門を commit する側へ移し、`useEngineSeat` と同じ凍らせ方に揃えた                 | `8b55ae2e`（テスト1本、変異で確認） |
| 2 / 3             | 直した。選んでいるかを先に見る。F-38 への参照2箇所                                       | `a003f8b5`（テスト1本、変異で確認） |
| 7                 | 直した。母数を `checkNames.ts` へ出し、`ratchetIndex` と共有した                         | `765ebe6a`（4件とも変異で確認）     |
| 5 / 6 / 8 / 11〜15 | 直した                                                                                  | `29706940`                          |
| 10                | 直した。寸法を `waits.ts` へ出した（`provider.test.tsx` は 30秒 → 15秒）                 | `a2f8fc7f`（変異で確認）            |
| 16                | **直さない**（下記）                                                                     | —                                   |

### 所見16 について——直さない

`entities/engine` の barrel の件は、**ラウンド12 の所見17 が同じものを挙げて
`docs/IDEAS.md` へ送っている**（「判断: 6週間以内に着手しない」）。IDEAS 側には
選択肢2つと、どちらを選んでも `features/settings` と `features/engine-position-sync` に
波及することが書いてある。**記録された判断を、確かめずに覆さない**
——r21 で同じ形の誤りをして r22 で戻したばかり。

加えて、いま barrel へ載せると `provider.test.tsx` の `vi.mock("@/entities/engine")`
（barrel を丸ごと差し替える）が `startInfiniteAnalysis` を `undefined` にするので、
モックの張り方まで同時に決めることになる。IDEAS の「まとめて決める作業」の見立てどおり。

### 所見1 について

r23 で「出口を1本に畳んだ」のは**出口**であって、守れる**窓**ではなかった。
枠を落とす形の後始末は「落とす前にタイマーが起きた回」を守れない——
**着地が速いテストしか書いていなかったので、2ラウンド気づけなかった。**

### 所見14 について（記録の訂正）

`ref 18 → 5` は `grep -c "useRef("` の結果で、**`useRef<T>(` を1つも数えていなかった**。
正しくは 18 → 16、`useEffect` は 10 のまま。r23 の記録を訂正した（`29706940`）。
**測った数字は、書いた直後に同じコマンドで取り直すこと。**
