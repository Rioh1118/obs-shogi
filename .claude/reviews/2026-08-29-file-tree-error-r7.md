...
- 内容: 「UI 側の判定は設定の文字列と canonicalize したパスを比べているので
  ...
  写っているので、片方だけ直しても残る。
- **対応**（`1092a77`）: 理由を現在のもの（取り消せない操作を UI の判定だけに預けない）へ
  書き直し、片方は参照に縮めた。
...
| H-3  | rust                         | `EXTRA_GUARDS` のコマンド名だけ実在検査から漏れ、綴り違いで無効になる                                     | `a765bbd`                                                                                  |
...
- **comment**: `EXTRA_GUARDS` / `root_guard` のモジュール doc の範囲（`1092a77`）／
  `mv.rs` の順序の理由が参照先と別のことを言っていた（`1092a77`）／
  `validate_under_root` の「存否は隠せない」が親の存否の話だった（`1092a77`）／
  `isProjectRoot` の TSDoc が後半で自分を否定（`1092a77`）／
...
**この報告書の対応列に、事実でない帰属が6件あった。** しかも `0df3616` の
コミットメッセージは「各行の主張が実際にそのコミットの差分に入っていることを
`git show --stat` で1件ずつ確かめてから書いた」と書いている。
**確かめたのは11組で、上の6件はその中に入っていない。** 確認したという記述自体が
実態より広い。

`b526df4` が触ったのは7ファイルで、`.rs` も `.ts` も1つも無い
（`git show --stat b526df4` を1回見れば分かる）。上の6行は `1092a77` と `a765bbd` に
訂正した。

Rust のテスト件数（23件）も実測と合わなかった（実測18件）。`CLAUDE.md` が
「件数をここに書かない。書くと必ず腐る」と決めているので、数を消した。

**書き戻しの誤りは5ラウンド続いている。** ラウンド7 で「突き合わせてから書く」と
決めたが、突き合わせの網羅を確かめる手段が無かった。ラウンド8 からは
`git log --no-merges <base>..HEAD --stat` の出力を先に手元へ出し、
対応列をそこから**引く**（記憶から書かない）。

## 検証

- `npm run verify` — 緑（41ファイル / 333件）
- `npm run build` — 緑
- `npm run verify:rust` — 緑（件数は `cargo test` の末尾で確認する。ここに書くと腐る）
