...
**12件すべて着手——**と書いたが、**R26-M5 は半分しか直していなかった**
（ラウンド27 の R27-M4 で判明）。所見は `CONTRIBUTING.md` と
`.claude/hooks/verify-gate.sh` の2箇所を指していたのに、修正コミットは
前者しか触っていない。後者はラウンド27で直した。

| 所見                  | 直し方                                                                                | コミット  |
| --------------------- | ------------------------------------------------------------------------------------- | --------- |
| R26-H1 / M4 / M5      | `usiok` の綴りを揃え、**`timeout_marker` を新設**。`CONTRIBUTING.md` の表と手順も直す | `0d7a2c5` |
| R26-H2 / M6 / M10     | 満杯のとき**空いた枠**を落とす。`is_safe_to_retain` に改名し、全長を数えるのをやめる  | `065f57b` |
| R26-M1 / M2 / M3 / M7 | ラウンド25で数え漏らした4箇所を機構の現物に合わせる                                   | `b688824` |
| R26-M8                | 着手の送信失敗を絞りの枠から外す（`needs_every_line`）                                | `e023a61` |
| R26-M9                | `startGame` の失敗の分け方から断言を落とす                                            | `3ed5373` |

### two-strikes を満たしたのでラチェットにした

**R26-H1。** ラウンド25で綴りを5通りから1つへ寄せ、その回に1件取りこぼした。
`CLAUDE.md` の「同じ失敗を2回するまでルールを足さない。1回目はルールではなくテスト」
に従い、`EngineError::Timeout(` の実引数を走査する `timeout_marker` を置いた。
`#[cfg(test)]` を落とす走査は `production_unwrap` が持っていたので `scanning` へ寄せた。

`analyzer.rs` の1件（範囲外の #371）も同じ形に揃えた。目印を「起動段だけの決まり」に
すると、次に足す人がどちらの側か判断できない。

### 所見より弱く倒した1件

**R26-H2。** rust が示したテストは「連打の直後に本物の対局が自分の枠を取れる」を
要求していたが、そう作ると**連打そのものが素通りする**（新しい枠の先頭は必ず通るので、
毎回 evict → insert すれば1行ずつ出る＝R25-H2 へ戻る）。

**空いた枠だけを落とす**形にした。空いた枠は何も覚えていないので落としても
情報が失われず、連打の最中は誰も落とせないので絞りは効いたまま。
本物の対局は `REJECTION_WARN_INTERVAL` で枠が空けば取り戻す。
テストはその自己回復を見る形に書き換えた（枠の長さをテストから縮められるようにした）。

### 変異試験

すべて baseline 差分。

- `usiok` の綴りを戻す／走査の綴りを変える（空振り検出が拾う）／見本まで要求する
- 落とす口を消す（ラウンド25の形）／空いていない枠まで落とす（ラウンド24の形）／長さを見ない
- `moveDecided` を絞る側へ戻す
