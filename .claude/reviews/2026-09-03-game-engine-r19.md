...
28件（重複を除くと21件）すべてを処理した。

### 字句解析を1本にし、書けなくした

| 所見    | どう直したか                                                                              |
| ------- | ----------------------------------------------------------------------------------------- |
| R19-B1  | `without_comments` を `blank_out_comments` に置き換え。文字列の中の `/*` で残りを捨てない |
| R19-H4  | `find_in_code` で、コードの中の `#[cfg(test)]` だけを item の始まりとみなす               |
| R19-H6  | `signature_of` がジェネリクスを `matching` で飛ばしてから `(` を探す                      |
| R19-H7  | `use_statements` が `blank_out_noncode` を通してから括弧を数える                          |
| R19-M8  | `fields_in` に入る前に TS のコメントを落とす                                              |
| R19-M16 | `serde_naming` も同じ字句解析に載せ、**`no_test_counts_delimiters_by_hand` を足した**     |

**「1つに寄せた」を人の注意で担保するのをやめた。** 寄せるだけでは次の人が
手で数え直して同じ穴を開ける（現に開いた）。`tests/` の中で区切り文字を数えている形を
探し、`scanning` を使っていないファイルを落とす。免除は置かない。

### 対局が壊れる

| 所見    | どう直したか                                                                             |
| ------- | ---------------------------------------------------------------------------------------- |
| R19-M12 | 上限に当たったら**断らずに終局**（`Rule`）。断ると「アプリが裁定を返さなかった」に化ける |
| R19-M11 | `get` と `ids` も `closing` を見る。閉じている最中を「知らない対局」と言わない           |
| R19-M14 | `closing` を外す責任を `Drop` に持たせる                                                 |
| R19-M10 | `spawn` と `usiok` で残りを引き直す（1回だけ読むと締切を10秒はみ出す）                   |
| R19-H5  | `GamePhaseView` の3バリアントぶんの見本を並べる                                          |
| R19-M15 | `root_guard` の3箇所を `expect` に。走査の故障を「該当なし」に写さない                   |

### doc

R19-H1（`SEARCH_GRACE` の存在しない式）、H2（※3 と不変条件6 の矛盾）、
H3（`scanning` の「3つ」）、M1〜M7、M9、M13 をすべて現物と突き合わせた。
`comment_identifiers` の干し草に `tests/` を足した——`src/` の doc が
ラチェットのテスト名を指すのは正しい参照なのに落ちていた。

### 変異で確かめたもの

- doc コメントに `#[cfg(test)]` を書いた直後の `.unwrap()` → 落ちる（reviewer の変異）
- 文字列に `/*` を含む定数＋生パスのコマンド → 落ちる（同上）
- `fn f<F: Fn() -> String>(.., file_path: String)` → 落ちる（同上）
- コメント行の `mod tests {` → 段の検査が生きたまま落ちる（同上）
- `tests/` に手書きの `matches('{')` を足す → 門番が落ちる
- `GamePhaseView` の `over` から `result` を落とす → 落ちる
- 欄をコメントへ退避する → 落ちる
- `closing` を見ない `get` / `ids` に戻す → それぞれ落ちる
