...
**10件すべて直した。2コミット**（`710c24c5` / `HEAD`）。

| 所見     | 直し方                                                                                                 |
| -------- | ------------------------------------------------------------------------------------------------------ |
| **R2-A** | 旧 doc を削る。`None` の説明は `compact_bucket` にだけ置く                                             |
| **R2-B** | doc をテストの直上へ戻す                                                                               |
| **R2-C** | `install_restored` から `state` を落として `Updating` に決め打つ。`update` から書けることは doc に明記 |
| **R2-D** | `search.md` の5箇所を `restart(Restart::..)` へ                                                        |
| **R2-E** | 畳む呼び手を表に。**`with_files` はしきい値超えだけ、`compact_all_buckets` は全桶**                    |
| **R2-F** | 「作る側は3つ」。`decode_all` が release でも弾くことも                                                |
| **R2-G** | `from_soa` を削除。昇順の検査を迂回する扉を消した                                                      |
| **R2-H** | `one_file` を `bucketize_entries` 経由に。同じ桶に複数件のテストを1本                                  |
| **R2-I** | 段を具体の variant で書く。捨てることは器のテストで見る                                                |
| **R2-J** | `index_cache.rs` の3箇所                                                                               |

### 変異

| 変異                                 | 結果                               |
| ------------------------------------ | ---------------------------------- |
| `bucketize_entries` の並べ替えを消す | **2本落ちる**（直す前は0本）       |
| `From<Restart>` の腕を入れ替える     | **落ちる**（直す前は緑）           |
| `restart` を `update` にすり替える   | **落ちる**（直す前はテストが無い） |

### 途中で1回踏んだ

変異を戻すのに `git checkout <file>` を使い、**未コミットの `install_restored` の
変更を巻き戻した。** コンパイルエラーで気付いた。
**変異は `cp` で退避してから当てる**（他の箇所ではそうしていた）。

### 検証

- `npm run verify` **667 passed**、`npm run verify:rust` **通った**
- `cargo test --lib search::store` **17 passed**（r1 から +2）
