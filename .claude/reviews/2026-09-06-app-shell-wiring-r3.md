...
| 所見   | 結果     | コミット   | 一行                                                                                       |
| ------ | -------- | ---------- | ------------------------------------------------------------------------------------------ |
| r3-01  | 対応済み | `4158704d` | `Hand` は止めない（空の駒台を描く）ことを doc に書いた                                     |
| r3-02  | 対応済み | `4158704d` | `player` があれば `shogi` もあることと、`?.shogi` を残す理由を書いた。テスト名も直した     |
| r3-03  | 対応済み | `e7b5ac43` | catch にも `cancelled` のガード。効かない cleanup の setState を落とした                   |
| r3-04  | 対応済み | `4de4ed2f` | `(B2, E3)`（戻す側）のテストを足した。変異で落ちることを確認                               |
| r3-05  | 対応済み | `9d36f315` | `×`（起きない）と `—`（変わらない）に割り、`—` で ✓ 無しは未検証と凡例に書いた             |
| r3-06  | 対応済み | `7aeab1e7` | `lineNumberRefsIn` を広げ、`:29` を落とした。**機械が守る側に入った**                      |
| r3-07  | 対応済み | `4de4ed2f` | `(B1, E1)` / `(B1, E2)` / `(B1, E7)` を `location.key` の観測で埋めた                      |
| r3-08  | 対応済み | `9d36f315` | ※1 の経路を「reload でクエリごと復元」に差し替えた                                         |
| r3-09  | 対応済み | `9d36f315` | E2 の発生源を3つ書き、「閉じるボタンは無いが E2 は実在する」を添えた                       |
| r3-10  | 対応済み | `4158704d` | テストの doc の「唯一の綴り」を型の doc と同じ語に揃えた                                   |
| r3-11  | 対応済み | `c2635725` | テストの doc から、provider に本体がある説明の写しを落とした                               |
| r3-12  | 対応済み | `2a8dca32` | `loaded` / `hasBadges` を落とし、綴りを `hasKifu` 1つにした                                |
| r3-13  | 対応済み | `dbb4709c` | 冒頭を「レンダ例外では畳まれない」に限定。見出しの差し替えと #434 を書いた                 |
| r3-14  | 対応済み | `d334b338` | `board-loading` を SCSS の節から出し、`board.md` の P0 が嘘であることを明記した            |
| r3-15  | 対応済み | `d334b338` | 「5件」を数の書いていない形にし、6件目の出どころ（r2）を書いた                             |
| r3-16  | 対応済み | `68565251` | 「符号化を知っているのはここだけ」を落とし、関門が `shared` にあることを名指しした         |
| r3-17  | 見送り   | `7848ea13` | `docs/IDEAS.md` へ。閉じるなら走査ごと入れるべきで、それは別の PR                          |
| （新） | 対応済み | `ed261dd8` | **表に機械を掛けた。** 遷移を書いたセルは ✓ を持つか「埋まっていないセル」に載っていること |

### 足した走査が、人のレビューより多く見つけた

`ed261dd8` の走査を初めて走らせたとき、指したのは4セル。人のレビュー（r3-04）が
挙げたのは `(B2, E3)` と `(B1, E2)` の2つで、**`(B1, E7)` と `(B3, E1)` は誰も挙げていなかった。**
4つとも埋めてある。

### 計画からの逸脱

- **1所見1コミットを3回破った**（`4de4ed2f` = r3-04 + r3-07、`4158704d` = r3-01 + r3-02 + r3-10、
  `9d36f315` = r3-05 + r3-08 + r3-09）。いずれも同じ表の同じ節、または同じ doc ブロック1つで、
  別々に取ると中間状態が2〜3回できる
- 計画は順2（走査）を順3（セルを埋める）より先に置いたが、**走査は赤いままコミットできない**ので
  実際にはセルを先に埋めた（`4de4ed2f` → `ed261dd8`）
