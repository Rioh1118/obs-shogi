...
**BLOCK 1件と HIGH 15件、MEDIUM 24件のすべてを対応。** issue へ送ったものは無い。

**各行は `git grep` と `git log -- <path>` で差分に入っていることを確かめてから書いた**
（前ラウンドの書き戻しで、入っていない対応を「対応」と書いた反省）。

| #    | 結果                                                                                                                                                  |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| B-1  | 対応。`reducer.ts` のガードの理由を、いま生きているもの（対話の裏に箱が重なる）だけにした                                                             |
| H-1  | 対応。`await loadFileTree()` に戻し、結果だけ捨てる。6箇所とも印つき                                                                                  |
| H-2  | 対応。H-1 と同じ（待てば直列化される）                                                                                                                |
| H-3  | 対応。H-1 と同じ（待てば交差しない）                                                                                                                  |
| H-4  | 対応。`isNameInputError` に `invalid_extension` を足し、網羅 `switch` にした                                                                          |
| H-5  | 対応。`void f(` とメンバ呼び出しも拾う。既存の9箇所には理由つきの印を付けた                                                                           |
| H-6  | 対応。span への移動をやめ、落ちた名前のまま外へ出たら編集を閉じる。テストも実際の経路にした                                                           |
| H-7  | 対応。`padding="none"` を実際に渡した。30px の因果も `Form` の padding だと書き直した                                                                 |
| H-8  | 対応。薄める先を親の面にし、擬似クラスでは掛けずに置き換える。`:disabled` は測らない                                                                  |
| H-9  | 対応。色が解けない宣言も `unmeasured` に数える                                                                                                        |
| H-10 | 対応。`src-tauri/tests/root_guard.rs` へ移し、ディレクトリを実行時に列挙。両方の属性表記・行コメント・件数の下限に対応し、検査自身のテストを2件足した |
| H-11 | 対応。`get_file_tree` に `AppHandle` を足して `validate_under_root` を通す。`EXEMPT` は空になった                                                     |
| H-12 | 対応。`isRaisedFromModal` を網羅 `switch` で `features/file-conflict/lib` に置いた                                                                    |
| H-13 | 対応。`"analysis"` を落とし、`modalTypes.test.ts` で対応を保つ                                                                                        |
| H-14 | 対応。ADR-0006 の決定と諦めたものの両方に書いた。単一 Now 前提の記述2箇所も直した                                                                     |
| H-15 | 対応。境界を「`main` に入った後」に書き直した。同じ PR の中では実装が決定を覆すことがある                                                             |
| M-1  | 対応（H-6 で span への移動ごと戻したので、その理由も消えた）                                                                                          |
| M-2  | 対応。`root_guard.rs` の doc から経緯を落とした                                                                                                       |
| M-3  | 対応。3箇所とも「通知（`state.error`）か衝突の対話（`state.conflict`）」と書き分けた                                                                  |
| M-4  | 対応。`TODO(#184)` から件数を落とした                                                                                                                 |
| M-5  | 対応。「src と dest の2つを同時に受ける唯一のコマンド群」に直した                                                                                     |
| M-6  | 対応。「行き先は5つ。名前を持つのは3つ」に直し、`deleteNode` が直に積む理由も書いた                                                                   |
| M-7  | 対応。TSDoc を `submitRename` の直上へ                                                                                                                |
| M-8  | 対応。件数を書かず範囲を書く形にし、モーダル側の3つが通っていないことを → #224                                                                        |
| M-9  | 対応。`CommitOutcome` を入力欄まで通し、成功と「ここには出さない失敗」を型で分けた                                                                    |
| M-10 | 対応。`isLoading` に揃えた                                                                                                                            |
| M-11 | 対応。`canRename` と `renameLabel?` を落とし、対話の2つの分岐も外した                                                                                 |
| M-12 | 対応（H-1 で待つように戻したので、背景の読み直しと対話が交差しなくなった）                                                                            |
| M-13 | 対応。不変条件1に「自分の操作については読み直しを待つので守れる」を足した                                                                             |
| M-14 | 対応。`commitName` を `entities/file-tree/lib` へ下げた                                                                                               |
| M-15 | 対応。`shared/lib/overlayStack` に切り出し、付箋・コンテキストメニュー・フォークメニューも載せた                                                      |
| M-16 | 対応。`src/__tests__/walk.ts` に寄せ、`__tests__` を含める既定と例外の理由をそこに書いた                                                              |
| M-17 | 対応。`Select.scss` の基底を暗い面へ畳んだ                                                                                                            |
| M-18 | 対応。`StudyPositionSaveModal` の `.form__*` の私物の上書きを落とした                                                                                 |
| M-19 | 対応。箱を1行に留め、全文は `title` で読めるようにした（覆う面積を最小にする）                                                                        |
| M-20 | 対応。ホバーは文字の濃さでも示し、選択中は明示で勝たせる                                                                                              |
| M-21 | 対応。`ButtonGroup.align` / `.inline-name-editor__input` / `.node-box--editing` を落とした                                                            |
| M-22 | 対応。`src` の root 検査を存在確認の後ろへ（`operations.rs` と順序を揃えた）                                                                          |
| M-23 | 対応。`dest_dir` / `dest_parent` を存在確認より前に通す                                                                                               |
| M-24 | 対応。`root_dir` 未設定で関門が開くことを、関門とテストの両方に書いた                                                                                 |

そのほかの文書側も対応:
改名した `deferFailure` のコメント／ADR-0001 本文への撤回の印／部分 supersede の書式／
#214 と #215 を該当ファイルから `TODO` で辿れるように／`CONTRIBUTING.md` の意味色の表と
検査・逃げ道の一覧／`failure-surfacing.md` §1 の件数を書かない形へ／`F-12a` / `F-12b` の採番／
`file-tree.md` の S3 / S5 の in-flight。

### 自分が作った退行（先に書く）

**ラウンド3 の対応が、ラウンド4 で HIGH 6件を生んだ。**

| ラウンド3 の対応             | ラウンド4 で出たもの                                         |
| ---------------------------- | ------------------------------------------------------------ |
| H-4 読み直しの失敗を返さない | **H-1 / H-2 / H-3** 待つのをやめたので、ツリーが古いまま返る |
| M-4 / M-5 確定を1箇所へ      | **H-4** `invalid_extension` を落とし、打った文字列が消える   |
| H-7 `AsyncResult` の検査     | **H-5** 同じラウンドが入れた `void f()` を見ていない         |
| H-2 Escape を span へ        | **H-6** span には届かない。テストが実在しない発火元を使う    |

**加えて、コミット本文が主張した内容が差分に入っていないものが3件あった**（H-7 と §4 と件数）。
どれも「直したつもり」を確かめずに書き戻したことが原因。

### 足した装置

- `src/__tests__/modalTypes.test.ts` — 誰も描かない `ModalType` を止める
- `src/__tests__/walk.ts` — 走査対象を1箇所で決める
- `src/shared/lib/overlayStack.ts` — 重なりの順序（Escape と閉じ込め）
- `src-tauri/tests/root_guard.rs` — `validate_under_root` の呼び忘れ。検査自身のテストつき
- `contrastRatchet` の `opacity` と `unmeasured` の数え方
- `commentHistory` にレビュー識別子（`// 6:` / `(C-H1)`）

テストは **31ファイル / 269件 → 33ファイル / 275件**。Rust は 5 → 7件（うち3件は検査自身）。
