...
4コミット＋ issue 1件。`npm run verify` / `npm run verify:rust` とも green。

| 所見       | 直した内容                                                                                                                                                                     | コミット  |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------- |
| **R12-H1** | 「もう無い席」の停止を成功にする。他人の席は止めないまま                                                                                                                       | `a24bd92` |
| **R12-B2** | `is_active` を落とす。席が在ること自体を「走っている」に                                                                                                                       | `a24bd92` |
| **R12-B1** | ※11・P0 と docs 4ファイルの `is_active` を現物に合わせる                                                                                                                       | `a24bd92` |
| **R12-B3** | `stop_analysis` が畳まれるまで待つ／無限解析に drain／`sessionId` を照合                                                                                                       | `9c43a07` |
| **R12-M1** | `verify-gate.test.sh` を `npm run verify` から呼ぶ                                                                                                                             | `c888a71` |
| **R12-M2** | `LINE_SUFFIX` に範囲の並びを足す                                                                                                                                               | `c888a71` |
| **R12-M3** | `kind_of` の単射を見る。`Kind::ALL` の doc を条件付きに                                                                                                                        | `c888a71` |
| **R12-M4** | `commentHistory` が `.claude/hooks/` と `#` コメントを読む                                                                                                                     | `c888a71` |
| MEDIUM 7件 | `StopVerdict` の分岐、`collect_until_bestmove` の帰結、`send_command` のエラー種、`new_session_id` の因果、`verify-gate.sh` の列挙、`docsSourcePaths` の件数、`#[tokio::test]` | `4b4974f` |

呼び出し側0件の経路（`analyze_with_time` / `analyze_with_depth`）の5件は → **#371**。

### 直し方を変えたもの

**R12-B3** は所見が「`stop_analysis` に `bestmove` を待つ相を持たせる」だったが、
`stop_analysis` は `raw_rx` を持っていない（読むのはストリームのタスク）。
タスクが抜けたことを `Notify` で知らせ、`stop_analysis` はそれを上限つきで待つ形にした。
待つのは `stop` を**書けた**ときだけ——積み置きを落としたときや送れなかったときに
待つと、来ない `bestmove` を3秒待って停止が遅れる。

**R12-M4** は所見が「`ROOTS` に `.claude/hooks/` を足し、`sourceFiles` に `.sh` を含める」
だったが、それだけでは足りなかった。コメントの走査が `//` と `/* */` しか見ないので、
`.sh` を歩いても `#` は1行も拾えない。**足しただけで守った気になる形**だったので、
コメントの綴りを拡張子で選ぶようにした（全部に `#` を掛けると Rust の
`#[derive]` が丸ごとコメント扱いになる）。

### 変異で確かめたもの

| テスト                                             | 当てた変異                          | 落ちた      |
| -------------------------------------------------- | ----------------------------------- | ----------- |
| `stopping_a_session_that_is_already_gone_succeeds` | r11 の「もう無い席も `Err`」に戻す  | ✓           |
| `only_a_written_stop_is_worth_waiting_for`         | 常に待たない／常に待つ              | ✓（両方向） |
| `lineNumberRefsIn`                                 | 並べた側から範囲を落とす            | ✓           |
| `commands_covers_every_gui_command`                | 2つの `GuiCommand` を同じ `Kind` へ | ✓           |
| `変更の経緯を書いていない`                         | `.sh` に経緯を戻す                  | ✓           |

### 自分が作った退行（ラウンド11のもの）

- **`analysis.md` ※11 を、同じラウンドの中で自分で嘘にした。** `6c50c03` で直し、
  2コミット後の `d0d2b04` でコードを変えた。**報告書に「R11-B1 直した」と
  書いた時点で既に偽。** `docsIdentifiers` は綴りの実在しか見ないので緑で通る
- **`stop_session` の照合と `forward_results_to_ui` の `remove` が、単独では正しく
  組み合わせで壊れた。** エンジンが死んだときの真因が「知らない ID」に置き換わり、
  フロントの再開が `catch` に落ちて解析が始まり直さなくなっていた
- **`is_active` を偽にする最後の口を消した。** 欄が定数になり、それに乗っている
  3箇所の判定が意味を失った
- **`drain_stale` を、唯一フロントが使う無限解析には掛けなかった。**
  掛けたのは呼び出し側0件の2本だけ
- `verify-gate.test.sh` を走らせる口を作らなかった。「機械にした」と書いたものが
  手で1回走らせただけの状態だった

### 範囲について（次のラウンド）

件数が **20 / 17 / 16 / 15 / 16 / 20 / 20 / 20 / 28 / 33** と増え続けている理由は、
**対象が2つに割れていて、呼び出し側0件のほうに手を入れ続けたこと。**

`game/session.rs` の本体（1000行超）と `game-session.md` の全セルは
**r7 から6ラウンド持ち越し**。このブランチの中心なのに、毎ラウンド
`analyzer.rs` に押し出されている。

**ラウンド13は `analyzer.rs` の解析側を範囲から外し、対局側だけを見る。**
