...
**16件＋小さいもの2件、全部直した。** 送り先に回したものは無い。

| 所見                                                 | 結果                                                                                                                                                                                     |
| ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MEDIUM-4 / MEDIUM-5 / MEDIUM-9 / MEDIUM-7 / MEDIUM-8 | `bc75fee4`。三項の順を入れ替え、`isRecoverableNotReady` と `TerminalNotReadyReason` を engine に置き、表の名前を「いつ出すか」の軸に揃え、ラチェットの表の一覧をソースから引くようにした |
| HIGH-3                                               | `988dd1e1`。`deletePreset` の2つの dispatch を同じ同期区間へ。`entities/engine-presets` にテストを新設                                                                                   |
| MEDIUM-6                                             | `bc75fee4`。文言を「選び、場所を設定してください」に広げ、`types.ts` の `no-engine` の doc も直した                                                                                      |
| BLOCK-1 / HIGH-2                                     | `bc75fee4`。テストの表題とコメントを state の話に戻し、effect のコメントを実際に効いている理由へ書き直した                                                                               |
| MEDIUM-10 〜 MEDIUM-16 と小さいもの2件               | `2526a7ae` と `bc75fee4`                                                                                                                                                                 |

**変異を当てて確かめたもの**——三項を元の順に戻すと「選択が外れていれば、初期化に
失敗していても no-engine」が落ちる。`deletePreset` の dispatch を分け直すと
「runtimeConfig は null を通らない」が落ちる。

**深刻度の割れ（HIGH-3）は「畳む」側に倒した。** react の「断つのは正しい」も成り立つが、
`deletePreset` の窓は r1 以前から**無駄なエンジンの畳み直し**を起こしており、
断りの文言以前に直すべきもの。畳んだ結果、この操作でも解析は新しいエンジンで読み直す
——カードで切り替える経路と同じ結末になった。
