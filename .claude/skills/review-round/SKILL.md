...
**範囲が広すぎると所見が薄くなる。** 変更ファイルが 30 を超えるなら、まずサブシステム単位に割り、
どれから見るかをユーザーに確認する。勝手に全部走らせない。
...
**そのまま修正に入らない。** 何から直すかはユーザーが決める。
`/review-fix` で修正ラウンドに入る。

## 禁止事項

- 所見の水増し。LOW を並べて件数を作らない
- 読んでいないファイルについて書く
- 「テストが通っているので問題ない」と書く（TS 側は片手で数えられる本数 / Rust 0件しかテストが無い）
